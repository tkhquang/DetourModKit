...
namespace DetourModKit
{
    ...
    namespace Anchors
    {
         ...
         *          cascade pattern bytes and decode parameters, the @ref StringXref literal and shape flags, or the
         *          @ref AnchorKind::Manual literal) -- and deliberately excludes any resolved address. Two anchors that
         *          resolve the same target through the same evidence therefore share a fingerprint even when the target
         *          moved to a new address between game versions, so a future manifest diff can tell "same evidence, new
         *          address" (expected drift the anchor self-healed) from "new evidence path" (the signature itself was
         *          rewritten). A candidate's cosmetic @ref Scanner::AddrCandidate::name is excluded because it never
         *          affects which address resolves. For @ref AnchorKind::Quorum the two sub-anchors' fingerprints are
         *          combined order-independently with the agreement mode and tolerance; a null sub-anchor contributes a
         *          fixed sentinel. @ref AnchorKind::CallArgHome has no resolvable evidence yet, so its fingerprint
         *          reflects only the kind.
