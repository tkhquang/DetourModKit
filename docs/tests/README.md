...
A test that must observe a guarded primitive contain a real hardware fault needs a committed `PAGE_NOACCESS` page held until process teardown. It must never be released with `MEM_RELEASE`, or a recycled virtual address flakes the fault onto live memory. These fixtures do NOT join the `tests/test_*.cpp` glob. [docs/design/testing.md](../design/testing.md) owns the placement rule and the no-toolchain-gate rule.

- Reusable fixtures live in [`tests/fixtures/fault_injection.hpp`](../../tests/fixtures/fault_injection.hpp). `dmk_test::NoAccessPage` is a leaked-on-purpose committed no-access page. `dmk_test::ProtectedPage` is a page pinned to a chosen protection, with `current_protection()` to check that a fault path restored it. `dmk_test::ExecutablePage` is a zero-filled RWX page usable as a synthetic module image, with helpers to plant a literal and a RIP-relative `lea`.
