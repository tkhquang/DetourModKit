...
- **Canonical runner -- `ctest`:** `gtest_discover_tests(... DISCOVERY_MODE PRE_TEST)` registers each gtest case as its own ctest test, so `ctest` runs every case in a separate process. That per-case isolation is the canonical green/red signal: a few suites drive process-global singleton state -- the `ConfigTest` log-capture cases reconfigure the one `Logger::get_instance()` sink -- and can interleave when many cases share one monolithic `DetourModKit_tests.exe` process. Use the standalone exe for fast local iteration and `--gtest_filter` debugging, but validate with `ctest`.
