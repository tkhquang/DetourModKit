...
A proof that needs a real loader transition (`LoadLibrary`/`FreeLibrary` reference-count behavior, `DLL_PROCESS_DETACH`) or a controlled static-teardown ordering cannot run inside the monolithic GoogleTest process, and one of them replaces the global allocation operators for its whole process. These live in `tests/lifecycle/`. [`tests/lifecycle/CMakeLists.txt`](../../tests/lifecycle/CMakeLists.txt) builds each as a CMake-owned target and registers it as a `lifecycle-proof`-labelled ctest whose verdict is the process exit code (0 = pass, 1 = proof failure, 2 = setup failure). Build and run with `bash scripts/run_lifecycle_proofs.sh`, or build the lifecycle targets and run `ctest -L lifecycle-proof` directly. Gated to MinGW; MSVC loader and native-TLS behavior requires separate fixtures.

- `bootstrap_probe_dll.cpp` -- a minimal mod-shaped DLL whose `DllMain` forwards attach/detach into `bootstrap()`/`bootstrap_detach()`, linked against the archive (the `bootstrap_probe` target). It is the only lifecycle target that links the library, so it is the only one instrumented for coverage.
...
The fault and lifecycle wrappers prepend the directory of the build tree's configured `CMAKE_CXX_COMPILER` before launching CTest. This prevents a probe DLL from binding an incompatible `libwinpthread-1.dll` supplied by another MinGW distribution earlier on the caller's `PATH`.
