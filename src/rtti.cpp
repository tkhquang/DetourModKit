...
namespace DetourModKit
{
    ...
    namespace
    {
        // Upper bound on distinct sub-object vtables collected for one mangled
        // name. Multiple/virtual inheritance produces a handful at most; the cap
        // keeps the reverse scan allocation-free (matches live in a stack array)
        ...
        // A validated reverse-RTTI hit: the vtable and the COL.offset of the
        // sub-object it belongs to (0 == primary/most-derived).
        struct VtMatch
        {
        ...
        // Enumerates the module's readable, non-executable, non-discardable
        // sections -- where MSVC keeps vtables and their RTTI meta-pointers
        // (.rdata for a normal /GR image, .data for a packed or section-merged
        // one). .text is skipped on purpose: a vtable's [-1] COL meta-slot never
        // lives in executable code, and sweeping code pages qword-by-qword would
        // multiply the one-time cost for nothing. Returns the number of ranges
        // written into @p out; 0 means the PE headers could not be parsed and the
        // caller falls back to the whole module image. Every header field is
        // bound- and signature-checked so a malformed or hostile image fails
        // closed instead of being read through.
        std::size_t collect_rtti_scan_ranges(Memory::ModuleRange mod,
                                             ScanRange *out, std::size_t cap) noexcept
        {
            ...
            // corrupt and would otherwise let the loop below run away.
            ...
            // optional header, whose length is SizeOfOptionalHeader. Using
            ...
            for (std::uint32_t i = 0; i < num_sections && count < cap; ++i)
            {
                ...
                // Use the in-memory extent (VirtualAddress + VirtualSize), never
                // the on-disk PointerToRawData/SizeOfRawData: those are file
                // offsets and do not survive section alignment once mapped.
        ...
        // Byte-exact comparison of the NUL-terminated name at @p name_addr with
        // @p mangled, including the terminator so a superstring does not match.
        // Mirrors vtable_is_type's name check; the caller guarantees
        // mangled.size() < MAX_TYPE_NAME_LEN.
        bool name_equals(std::uintptr_t name_addr, std::string_view mangled) noexcept
        {
        ...
        // Sweeps one [begin,end) window for vtables whose RTTI name equals
        // @p mangled, appending validated, deduped matches to @p out (capped at
        // @p cap). The window is read in page-bounded chunks (one guarded read per
        // page) and scanned in-process, so the guarded-read count is per-page
        // rather than per-qword -- the difference between a few hundred and a few
        // hundred thousand guarded transitions over a multi-megabyte section. A
        // meta-slot is a qword that points to an in-module COL; the vtable that
        // owns it is slot + 8, validated by the same COL prelude the forward
        // walker uses, so the in-range pre-filter only spares a deeper read and
        // never decides correctness.
        void sweep_range_for_name(Memory::ModuleRange mod, std::uintptr_t begin,
                                  std::uintptr_t end, std::string_view mangled,
                                  VtMatch *out, std::size_t cap, std::size_t &count) noexcept
        {
            // Image-resident pointer storage is 8-byte aligned, so only
            // qword-aligned slots can hold a vtable meta-pointer.
            std::uintptr_t addr = (begin + 7) & ~static_cast<std::uintptr_t>(7);

            while (addr + sizeof(std::uintptr_t) <= end && count < cap)
            {
                // Never let one guarded read cross a page boundary: an unmapped
                // page then fails only its own chunk, not the whole window.
                const std::uintptr_t page_end = (addr | Rtti::detail::PAGE_MASK) + 1;
                const std::uintptr_t chunk_end = (page_end < end) ? page_end : end;
                const std::size_t qwords =
                    static_cast<std::size_t>(chunk_end - addr) / sizeof(std::uintptr_t);
                if (qwords == 0)
                {
                    addr = chunk_end;
                    continue;
        ...
        // Finds vtables for @p mangled across the module's RTTI-bearing sections,
        // returning the count written into @p out (deduped, capped at @p cap).
        // Sweeps the readable non-executable sections; if the PE headers cannot be
        // parsed, falls back to the whole module image rather than reporting a
        // confident-but-wrong "not found" for a packed or merged binary.
        std::size_t scan_vtables_for_name(Memory::ModuleRange mod, std::string_view mangled,
                                          VtMatch *out, std::size_t cap) noexcept
        {
            ...
            if (range_count == 0)
            {
                // No usable scan window (the PE headers could not be parsed, or no
                // readable non-executable section qualified): sweep the whole image
                // rather than report a confident-but-wrong "not found" for a packed
                // or section-merged binary. The whole-image sweep is a strict
                // superset, and resolve_col_site still validates every candidate.
    ...
    std::optional<std::uintptr_t> Rtti::vtable_for_type(std::string_view mangled,
                                                        Memory::ModuleRange range) noexcept
    {
        ...
        // The primary vtable is the COL.offset == 0 sub-object: the value an
        // object pointer's first qword holds for a most-derived instance, i.e. the
        // faithful inverse of vtable_is_type. More than one distinct primary for
        // the same name (a type linked into the image twice) is ambiguous; fail
        // closed rather than return an arbitrary one.
    ...
    std::size_t Rtti::vtables_for_type(std::string_view mangled, std::uintptr_t *out,
                                       std::size_t out_cap, Memory::ModuleRange range) noexcept
    {
        if (!out)
            out_cap = 0;
        ...
        // Ascending COL.offset so the primary (offset 0) sorts first. The match
        // count is tiny (one per base sub-object), so an in-place insertion sort
        // is both adequate and allocation-free.
    ...
    {
        // Fast path: a completed resolve stored m_cached before publishing
        // m_resolved with release, so an acquire-load that sees m_resolved also
        ...
        // First use: resolve and cache. Concurrent first-callers converge on the
