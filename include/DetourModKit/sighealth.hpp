...
namespace DetourModKit
{
    namespace sighealth
    {
         ...
         *          @ref Robust.
         *          A byte record grades by its first declared rung because static lint cannot know whether that rung
         *          will resolve uniquely in the live scope; a manifest grades by its weakest record.
         */
         ...
         *          backend (RipGlobal / CodeOperand) fills @ref ladder and grades by its first rung; a text backend
         *          (StringXref / VtableIdentity) grades by @ref anchor_text_bytes. @ref label is owned by the report,
         *          and per-rung findings remain available in @ref ladder.
         */
        struct RecordHealth
        {
            /// The signature's key.
         ...
         * @param policy The grading thresholds.
         * @return The record's health. A byte backend grades by its first declared rung; a text backend by its
         *         anchor-text length; a Manual pin and non-serializable kinds report a record-level finding.
         * @note Setup/control-plane only: allocates, reads no process memory.
         */
