...
An installed header carries no seam and no private test macro. A Debug program mixes macro-on archive objects with macro-off examples and consumers. A macro-gated member therefore causes a formal ODR violation. `scripts/check_header_hygiene.py` rejects `DMK_ENABLE_TEST_SEAMS` and `DMK_EVENT_DISPATCHER_INTERNAL_TESTING` under `include/`. This gate keeps each installed definition token-identical with each macro on and off. Tests use unconditional friend accessors: `detail::LoggerTestSeams`, `detail::InputTestSeams`, and `detail::EventDispatcherTestAccess`. The related definitions reside in `src/internal/logger_test_seams.hpp`, `src/internal/input_test_seams.hpp`, and `tests/test_event_dispatcher.cpp`.
