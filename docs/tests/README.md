...
The `script-lint` label carries ten standalone Python self-tests: `HeaderHygieneStripperSelfTest`, `EmitTlsCheckerSelfTest`, `ExportEqualityGateSelfTest`, `MechanicalStyleCheckerSelfTest`, `TestLabelInventoryGateSelfTest`, `LifecycleSoakRestorationSelfTest`, `RuntimeDirResolverSelfTest`, `BenchmarkResultsCheckerSelfTest`, `WorkflowTopologyCheckerSelfTest`, and `GTestExecutionCheckerSelfTest`, plus `WorkflowTopologyIsBlocking`, which runs the topology gate against this repository's own workflows, and `SameBaseReplacementExecutionEvidence` (below). Python is a tests-ON configure prerequisite because the RTTI generation fixture needs it, so every configured test tree registers all of them; MinGW also registers the `EmitPathHasNoEmulatedTls` archive gate, which rejects winpthreads thread identity from the input binding teardown owner path.

A required step is unconditional. A step that does not run cannot fail, so a step-level `if` on a required command is a guard reporting green without having run it, and the checker refuses one on any step of a blocking quality or required release job. The exception is a frozen `REVIEWED_STEP_CONDITIONS` table keyed by the exact `(workflow, job, step)` tuple: those conditions either strengthen the step (`!cancelled()`, so a later gate still reports after an earlier failure) or guard a diagnostics upload, and none can skip a guard on an otherwise passing run. The table is checked by exact value, so a reviewed step cannot keep its name and take a false condition instead, and every entry must name a step that really carries it, so a stale entry cannot become standing permission for a step nobody reviewed. Both lifecycle soaks and the publish ref guard are additionally pinned as exact commands, which is what makes them uniquely identifiable rather than merely present.

Exit status must reach the job. `|| true` was never the only way to discard a failure -- any fallback that exits zero does it, and `|| echo ignored` is exactly as green -- so `||` is refused by construction on a required step rather than by enumerating the fallbacks somebody might reach for. The one carve-out decides nothing: shell test compounds joined inside a conditional head, where both sides are `[`/`[[` tests rather than the work the step exists to run. The same rule covers a check that detects a broken state and only reports it: a PowerShell `Write-Warning` leaves the step green, so both producers' empty-install-prefix check now uses `Write-Error` and a nonzero exit, and the warning form is refused.

Each step is read under the shell it will really run: the declared `shell`, or the job's runner default, which is PowerShell on `windows-*` and bash elsewhere. That decides both which status rules apply and which continuation marker joins physical lines, and the marker is not cosmetic. Bash removes a backslash-newline and PowerShell a backtick-newline without inserting whitespace, so a command name can be split as `Write-` plus `Warning`; reading a PowerShell step with bash's marker would leave the two halves in separate logical lines and match neither.

[`scripts/test_check_workflow_topology.py`](../../scripts/test_check_workflow_topology.py) has 69 tests. A stdlib-only structural reader scopes workflow, job, and step keys by indentation, so a nested `needs`, a spaced `if :`, a mode-default literal in another input, or a credential after `jobs` cannot answer for the direct key being checked. The positive controls cover the repository, block-sequence dependencies, the three reviewed captured substitutions, and harmless policy words inside quoted shell data. Negative mutations exercise every required release job plus the blocking quality and SIMD routes; exact producer-proof commands and their configure/build/CTest chains; the compiled benchmark-ledger build and self-test; required dependencies; publish mode, main-ref, candidate-SHA and exact tag-target guards; dot and bracket secret syntax; and known fail-open shell forms. The conditional-head carve-out requires every `||` operand to be a complete bracket-test compound, and PowerShell warnings are rejected at nested command boundaries as well as at the start of a line. A quoted capture whose fallback is not `true` or `:` has its own mutation, because that whole assignment lexes as one shell word and the `||` token scan never sees the operator: only the capture rule refuses it, so nothing else would catch that rule being narrowed back. Each negative asserts the refusal message, not only the exit code, and the process-boundary control proves both CLI exit statuses.
