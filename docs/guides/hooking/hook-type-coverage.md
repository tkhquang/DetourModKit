...
Gamepad interception treats the primary `XInputGetState` export and every distinct ordinal-100 `XInputGetStateEx` target as one coverage transaction. Both hooks are created disabled before either prologue is patched, so a creation failure rolls the pair back and leaves both entries open. A proxy-forwarded ordinal target receives its own pre-acquired module keepalive before either hook is published. If the ordinal-100 member exists but fails to arm, coverage is degraded rather than successful: suppression stays inactive, both detours pass through unchanged, and the owning poll loop keeps retrying the arm under a delay that grows to a cap and resets when the target module, the layer owner, or the export's own bytes change. Suppression becomes possible only when both members are armed. An absent or aliased ordinal-100 export is complete coverage because there is no second entry point to mask.
