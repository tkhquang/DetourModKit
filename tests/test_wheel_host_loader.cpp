...
namespace
{
    ...
    std::atomic<int> g_route_health_calls{0};
    std::atomic<int> g_retarget_calls{0};
    std::atomic<std::uint32_t> g_route_health_state{DMK_WHEELHOST_ROUTE_READY};
    ...
    void reset_stub_counters() noexcept
    {
        ...
        g_route_health_calls.store(0);
        g_retarget_calls.store(0);
        g_route_health_state.store(DMK_WHEELHOST_ROUTE_READY);
    }
    ...
    int32_t DMK_WHEELHOST_CALL stub_route_health(void *ctx, std::uint32_t *out_state, std::uint32_t *out_thread_id,
                                                 std::uint64_t *out_mount_generation) noexcept
    {
        if (ctx != &g_stub_context || out_state == nullptr)
        {
            return DMK_WHEELHOST_ERR_INVALID;
        }
        g_route_health_calls.fetch_add(1);
        // The stub is mounted on a fixed synthetic thread. g_route_health_state selects what it reports.
        *out_state = g_route_health_state.load();
    ...
    int32_t DMK_WHEELHOST_CALL stub_retarget(void *ctx, DmkWheelLease lease,
                                             std::uint32_t /*target_thread_id*/) noexcept
    {
        ...
        // A stub route that is not ready also refuses the remount, so the client latches one settled state
        // instead of oscillating between the reported state and a successful retarget.
        return g_route_health_state.load() == DMK_WHEELHOST_ROUTE_READY ? DMK_WHEELHOST_OK
                                                                       : DMK_WHEELHOST_ERR_THREAD;
    }
    ...
    DmkWheelHostTable make_stub_table() noexcept
    {
        ...
        table.route_health = &stub_route_health;
...
TEST(WheelHostLoader, RequiredHostCompatibilityFieldsAreValidated)
{
    ...
    DmkWheelHostTable table = make_stub_table();
    table.struct_size = static_cast<std::uint32_t>(sizeof(DmkWheelHostTable) - 1u);
    input::Input::Settings settings;
    settings.wheel_backend = input::Input::WheelBackend::ExternalHost;
    settings.wheel_host = &table;
    settings.wheel_host_required = true;
    ...
    guard = stage_wheel_binding("wheelhost_required_no_health");
    ASSERT_TRUE(guard.has_value()) << guard.error().message();
    guard->release();
    table = make_stub_table();
    table.route_health = nullptr;
    settings.wheel_host = &table;
    result = mgr.start(settings);
    ASSERT_FALSE(result.has_value());
    EXPECT_EQ(result.error().code, ErrorCode::InvalidArg);
...
TEST(WheelHostLoader, RequiredHostMissingRouteFunctionRefusesStart)
{
    ...
    // ABI v2 requires route_health and retarget. A table that clears the ROUTE capability or nulls a v2 function is
...
TEST(WheelHostLoader, ExplicitTargetDrivesHostRouteHealthAndRetarget)
{
    auto &mgr = input::Input::instance();
    mgr.shutdown();
    reset_stub_counters();
    ...
    settings.wheel_target_thread_id = GetCurrentThreadId();
    ASSERT_TRUE(mgr.start(settings).has_value());
    ...
    EXPECT_GE(g_route_health_calls.load(), 1) << "the poll loop must query route health each cycle";
...
TEST(WheelHostLoader, WheelSourceHealthTracksTheExternalRouteState)
{
    ...
    g_route_health_state.store(DMK_WHEELHOST_ROUTE_RETRYABLE);
    EXPECT_EQ(wait_for(Health::Retryable), Health::Retryable);

    g_route_health_state.store(DMK_WHEELHOST_ROUTE_CLEANUP_BLOCKED);
