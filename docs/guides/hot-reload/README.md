...
| `hook::inline_at` / `mid_at` (per target address)      | Fails with `TargetAlreadyHookedInProcess` if still hooked and strict duplicate refusal is enabled | Drop the prior `Hook` handle first               |
...
- **`hook::inline_at` / `mid_at`**: NOT replace-on-duplicate when strict duplicate refusal is enabled. The same-kit ledger is keyed by target address, not by hook name, and a second `inline_at` / `mid_at` with `Options::fail_if_already_hooked = true` while a live or in-progress `Hook` still targets that address fails fast with `TargetAlreadyHookedInProcess` so a stale detour is never silently swapped. Drop the prior `Hook` handle before re-installing.
