...
The drift report and the manifest gate gained additive trust surfaces: `ResolvedAnchor::witness` (a live-image identity, physical source, and completeness per resolved entry), `anchor::anchor_trust_fingerprint` and `scan::image_identity`, and -- for a manifest that authorizes a write -- `SignatureRecord::expected_image_identity` (persisted as the optional `image_identity` key), which `GatePolicy::mutation_strict` checks against the live image when captured, alongside a header-threaded `resolve_and_gate(sigs, header, build_revision, policy)` overload that enforces the contract `revision`. A read-only port needs none of these, but the `ResolvedAnchor`, `SignatureRecord`, and `scan::Hit` size growth means a clean rebuild against the v4 headers, as for any static-library ABI change.
