...
green without having proved anything:
  ...
  - a ``--require``d gate name that no suite emitted
    ...
    for number, line in enumerate(text.splitlines(), 1):
        ...
        try:
            if fields[0] == "#GATE":
                ...
                metrics[name] = Metric(suite, name, _number(value, "value"))
...
def parse_metric_ratio(spec):
    """Split a NAME:MIN ratio specification."""
...
def build_parser():
    ...
    parser.add_argument("--require", action="append", default=[], metavar="NAME", help="gate name that must be present")
...
def main(argv=None):
            ...
            if name in metrics:
                problems.append(
                    "metric '{0}' appears in both {1} and {2}; captures may not override evidence".format(
                        name, metric_sources[name][2], path
                    )
                )
                continue
            metrics[name] = metric
            metric_sources[name] = (provenance, found, path)
        gates.extend(found)
    ...
    for name in args.require:
        if name not in present:
            problems.append("required gate '{0}' was never recorded".format(name))
    ...
    if args.metric_ratio:
        if not args.baseline:
            ...
            for name, floor in args.metric_ratio:
                ...
                current_provenance, current_gates, current_origin = metric_sources[name]
                if current_metric.suite != baseline_metric.suite:
                    problems.append(
                        "metric '{0}' belongs to current suite '{1}' but baseline suite '{2}'".format(
                            name, current_metric.suite, baseline_metric.suite
                        )
                    )
                current_scenarios = {
                    gate.name
                ...
                declared = DECLARED_RATIO_PROFILES.get(name)
                if declared:
