...
- **Anchor registry (deferred):** the declarative anchor registry (the former `anchors.hpp` / `profile.hpp`) and its suites (`test_anchors.cpp` / `test_profile.cpp`) were removed in the v4 scan clean-break and are deferred to a later release (master-plan item 17); the registry backends are exercised through the scan-resolver, code-constant, string-xref, and RTTI suites above.
