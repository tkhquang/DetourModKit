...
**Inline.** `hook::inline_at(request, detour)` returns a `Hook`. `Hook::enable()` then rewrites the target function's prologue to a JMP into a trampoline and lets the detour call the original through the typed trampoline. `hook::install_all(table)` is the declarative batch form over the same mechanism. It returns `Result<std::vector<hook::InstallOutcome>>` in table order rather than a single handle, and each successful row carries its own `Hook`.
...
The inline and mid paths run on the SafetyHook backend. The backend removes execute access from the target and trampoline pages during patch transactions. Every patch transaction keeps routed generated pages executable under the [hook contract](../../design/hooking.md#concurrency-model). A process-global vectored exception handler (registered on first use) fixes up the instruction pointer of any thread that faults inside the region under rewrite. A patch transaction suspends no thread. The idle proof of route reclamation suspends threads, as "Clean x64 mid teardown" in the [hook design note](../../design/hooking.md) states. This trap-plus-IP-fixup model is a correct one for install and removal of inline patches in a live process. It reaches the same safety goal as the suspend-and-fixup discipline of Microsoft Detours and MinHook without a stop of the world.
...
- Original clears any stale backend flag and authorizes backend destruction. The [hook design note](../../design/hooking.md) owns published-route reclamation, retention, and leak accounting under "Clean x64 mid teardown".
...
- a wrapper that calls the destination and decrements on return,
- a separate non-mid wrapper that counts the closed bypass through provider completion, and
- an exit thunk that a mid stub returns through.

Windows x64 has no frame pointer to fall back on. The backend therefore registers `RUNTIME_FUNCTION` and `UNWIND_INFO` records for those regions before it publishes the route. The gateway and exit use a flag-transparent `push rbx; pushfq` frame and restore with `popfq; pop rbx` before their jump or return. Each wrapper uses a fixed 40-byte frame. A native exception raised inside a routed destination therefore unwinds correctly. A stack walk taken while a thread sits in any generated region reports the real caller. Mid-hook callbacks can observe and replace the intercepted arithmetic flags. Registration failure fails the create. A failed unregistration conservatively retains the referenced arena. A published route keeps its code and records together until the idle proof at teardown withdraws both. A retained route keeps both, so a thread that faults in it after the handle is gone still finds its records. SafetyHook's dynamic-RSP mid stub is the one exception: it is not described, which is why a mid-hook callback must not let an exception escape.

A published route is freed at teardown unless the idle proof fails, and retained storage is bounded rather than unlimited. Each route uses an isolated allocator arena. The backend counts the whole retained chain both as requested logical bytes and as the complete allocator blocks that back those requests. The chain is the gateway and its unwind records, the trampoline, and the optional mid stub. Separate reserved, charged, capacity, and monotonic high-water totals bound both dimensions. Creation reserves the worst-case chain before publication. The hook design note states the reservation size under "Clean x64 mid teardown". Publication charges the actual chain and refunds the remainder. Reclamation refunds the charge, and never-published destruction releases the reservation. Public `mid_at` obtains that reservation automatically. XInput interception explicitly reserves two credits before either pair member can publish. A ceiling that cannot hold both installs nothing rather than strand primary-only coverage.

The hook design note owns the [stale-slot creation refusal](../../design/hooking.md#concurrency-model). Unsupported mid windows fail before publication. `Lifecycle.MidRouteStaleSlotRefusesCreation` verifies the refusal.
