...
- `veh_unmap_dll.cpp` / `veh_survives_unmap.cpp` is the guarded-read handler lifetime host. The fixture links the archive like a mod DLL and runs one hook generation under the selected teardown. The host links nothing from DetourModKit. It unloads the image, proves the unmap with `GetModuleHandleExW(FROM_ADDRESS | UNCHANGED_REFCOUNT)`, then dispatches a continuable `RaiseException` and a C++ throw. A first-position handler that outlived its image faults inside the dispatcher, so exit status is the oracle. `session` tears down through `~Session` with no `init_cache`. `hook-only` has no `Session` and calls `memory::shutdown_cache()`. `with-init-cache` is the positive control. The host skips with code 77 on MSVC, which contains guarded reads in frame-based SEH and installs no handler. A separate `WILL_FAIL` case rejects an unknown token.
