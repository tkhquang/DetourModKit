...
- `tests/test_hook_backend.cpp` covers the managed hook/backend transaction boundary. It covers post-commit reported failures, contained throws before and after inline/mid enable, and disable and rollback mutation. It covers conservative Foreign/Indeterminate recovery after a committed restore. It also covers safe teardown with a stale backend flag, allocation-failure-safe pin diagnostics, persistent emitted-patch provenance, zero-filled first-enable refusal, and preservation of Foreign bytes. The idempotency cases verify full published state for both reconciliation and Foreign refusal polarities. Test-only address-scoped seams run transaction cleanup before they return an error or rethrow, so each discovered GoogleTest is an isolated host-survival and state-reconciliation proof. `HookBackendCreate.ExhaustedWindowNamesTheAllocatorRefusal` exhausts the allocator's +/-2 GB window with reservations for a leaf target and an interior-jump target. It pins the named allocator refusal in the log. `HookBackendCreate.InteriorShortJumpReportsTheFirstContentCheck` pins the first content-check text for a short jump into an instruction interior under a free window. The `HookToggleWarning` cases pin the four deferred toggle warning texts.
...
- `input_reshape_retirement.cpp` is the T-INPUT-RESHAPE host. Each mode runs in a separate process.
  ...
  Each mode leaves the reshape entry as the final callable owner. Its capture destructor calls `Input::binding_count`. Lock-held destruction deadlocks, so CTest `TIMEOUT` detects it. Exit status rejects an absent destruction. It also rejects a destruction outside the reshape window or on a thread other than the caller, which together bound the disposal to the reshape call.
...
- `input_tls_exhaustion.cpp` is the delivery-marker admission host, and the only place where the marker's failure mode is reachable. A taken TLS index set is process-wide, so the shared unit process cannot undo it.
  - `exhausted` consumes every index before DetourModKit reserves its slot, makes two threads race the first owner registration, and requires both to report failure. It then requires hold true, hold false, and press delivery refusal to run no consumer code and to restore the gate state each edge found. One thread's unrecordable frame must leave another thread readable as not callback-entrant.
  - In `exhausted`, balancing-edge and retired-callable self-reentry must complete on an ordinary C++ thread with no index. It must also complete on a foreign `CreateThread` host thread whose depth store is refused, with the exact allocation-free Win32 identity. A regression there hangs, so the ctest timeout is the oracle.
  - `exhausted` then hands the indices back and requires that no index was published. The last registration that found no index must then retire and leave no owner. The real facade drive must reserve a fresh index and deliver its edge, so the refusal lasts only while no owner holds an index.
  - `available` is the positive control over the same drive on a process that never exhausted its indices. A third registration is the `WILL_FAIL` unknown-token control.
  - `store-failure` covers the branch that index exhaustion cannot reach: the reservation succeeds and only the per-thread depth store fails. A reserved index past the TEB's inline slots does that when its lazily heap-allocated expansion array cannot grow. It requires the frame to report itself unrecorded and the thread to keep its not-callback-entrant answer. Hold and press deliveries must run no consumer code and must restore the gate state they found. Then, with the store in service again, an ordinary delivery, its balancing edge, and a real facade drive must all succeed. A permanently broken store therefore cannot pass as a refusal proof.
  - `store-failure` also covers the half that must not be refused. A mandatory teardown span opened while the store fails must report no recorded depth, yet still make its own thread read as callback-entrant. It must leave a freshly started thread with a false answer and stop the entrant answer the instant the span ends. Those four together separate an exact fallback from a process-wide marker, which satisfies the first two and fails the third.
  - `consume-exhausted` registers a consume binding while every index is taken and a second one after the indices return. The published gamepad table must count only the second, so a binding that cannot deliver never swallows its trigger.
...
- `input_control_thread_shutdown.cpp` is the control-thread `[B-90]` host. A cardinality-changing `rebind` delivers a Hold balancing edge on the control thread while the poll thread waits on that delivery for the rebuilt binding. In `shutdown-in-release`, that callback calls `Input::shutdown()` after 30 ms, and a 5 s watchdog exit code of 42 reports a join. `release-only` is the control, with the shutdown call on the main thread after the release returns. `abandoned-premise` is the bounded negative cleanup control: the release parks on a worker thread after its shutdown call while the main thread abandons the premise.
...
- `cache_shutdown_stall.cpp` stalls one admitted cache reader across `memory::shutdown_cache()`. The shutdown must return after its drain deadline. It must retain the shard storage and the precommitted module reference under `[B-73]`. It must also record one `LeakSubsystem::MemoryCache` event. The reader must exit through retained state. A later init and shutdown cycle must release the module reference. This process isolates the retained state and deadline checks.
- `protection_ledger_first_use_oom.cpp`, `hook_ledger_first_use_oom.cpp`, `reaper_first_use_oom.cpp`, and `backend_allocator_first_use_oom.cpp` are the `[B-91]` accessor hosts. They share `first_use_oom_poison.hpp`, which replaces the plain and nothrow global allocation operators with an allowance counter. Each host asserts the fail-closed result and its latch past the poisoned window. The allocator host names the backend island header, so it links the backend. Each host has three arms:

  - `oom` refuses every allocation.
  - `constructor-oom` grants the object its storage and refuses the constructor's own allocation, which the MSVC STL performs for an empty `unordered_map` or `list`. It returns skip code 77 on an STL whose constructor allocates nothing.
  - `success` is the live control.
- `never_destroyed_storage.cpp` covers two `[B-47]` forms. `exit-during-shutdown` parks `shutdown_cache` inside its seam window, suspends every other thread as `ExitProcess` does, and exits through the CRT. This image's static destruction then meets a joinable cleanup handle. `ExitProcess` is not the oracle: on MSVC it skips an EXE's static destructors, and on MinGW a terminate inside it keeps exit status 0. `configure-after-static-destruction` reconfigures the process default logger from a namespace-scope destructor that runs after the destruction point of the module directory cache. The working directory is moved away first, and the log file must appear beside the module.
- `route_copy_dll.cpp` / `route_copy_protocol.hpp` / `route_copy_host.cpp` load two fixture DLLs, `route_copy_a` and `route_copy_b`. Each DLL links its own archive copy. `Lifecycle.RouteCopies*` verifies the process coordinator contracts in the [hook note](../design/hooking.md#process-route-coordinator). A separate `WILL_FAIL` case rejects an unknown token. Some cases add conditions:

  - `Lifecycle.RouteCoordinatorSurvivesParticipantUnload` verifies owner survival after unmap.
  - `Lifecycle.RouteCopiesRetainAfterCoordinatorTimeout` and `Lifecycle.RouteCopiesRetainArmedHookAfterCoordinatorTimeout` measure teardown under persistent contention. Both require one retained executable route, one record, and one warning. The armed case preserves the patch. The disarmed case preserves the original bytes.
  - `Lifecycle.RouteCopiesRetryDisarmedTeardownAfterCoordinatorRefusal` and `Lifecycle.RouteCopiesRetryArmedTeardownAfterCoordinatorRefusal` refuse the first teardown acquisition. Both require clean reclamation, original target bytes, no record, and no warning after the later acquisition succeeds.
  - `Lifecycle.RouteCopiesReconcileStaleStateBeforeLayerConflict` restores the original bytes under an armed older hook, then installs and disarms a newer layer. The older arm must return `LayerConflict`, report Disabled, and leave the bytes original. After the newer layer retires, the older hook must arm again.
...
- `Lifecycle.DestroyLockFailureRetainsRoute` uses that host to inject a recursive-mutex failure. Published and unpublished routes retain their executable storage and charge exactly once. A raw hook retains its trampoline and marks its process record retained.
- `Lifecycle.ReconciledRouteReleasesProcessRecord` uses that host to restore a route target outside the backend. After the owner reconciles its state, teardown reclaims the chain and removes the process record.
- `Lifecycle.RefusedTeardownNamesCoordinator` uses that host to refuse one teardown restore acquisition. The armed routed and raw patches each retain and report the coordinator as the cause.
- `Lifecycle.RefusedTeardownCompletesRetention` uses that host to refuse one teardown acquisition. An unpublished route converts its reservation into one charge. A raw record becomes retained, so the older layer below it disarms.
- `Lifecycle.ProofSectionHoldsOnlyItsTargets` checks that all six route targets reside in `.proof` and only padding precedes the first target. An incremental MSVC jump table before the targets fails the case.
- `tls_index_return.cpp` is the TLS ownership host. Each scenario runs in its own process and counts free indices in the process TLS bitmaps. A separate `WILL_FAIL` case rejects an unknown token.
  - `Lifecycle.MidEntryTlsIndexReturnsWithTheLastSlot`, `Lifecycle.EmitTlsIndexReturnsWithTheLastSubscribedDispatcher`, and `Lifecycle.DeliveryTlsIndexReturnsWithTheLastGate` require the exact return after the last owner retires and a callback on the re-reserved index. The emit and delivery cases require a tracked frame for that callback. The emit case also requires a second publish and a refused publish on one dispatcher to return their surplus registrations.
  - `Lifecycle.MidEntryTlsIndexStaysWithARetainedSlot`, `Lifecycle.EmitTlsIndexStaysWithALiveDispatcher`, and `Lifecycle.DeliveryTlsIndexStaysWithALiveGate` release another owner beside a live one and require the index to stay reserved.
  - `Lifecycle.MidEntryTlsIndexRefusalReleasesItsClaim` exhausts the process TLS set and requires `hook::mid_at` to refuse with `SystemCallFailed`. A later mid hook must then return its index exactly.
  - `Lifecycle.GuardedReadTlsIndexReturnsOnRelease` requires the MinGW guarded-read index to return on release and a fresh index after a foreign owner takes the old one. A refused arm store must route reads and writes to their fallback and fail a region closed.
  - `Lifecycle.GuardedReadTlsIndexWaitsForTheInFlightAccess` releases under a parked guarded region and requires the index to stay until the drain. Both guarded-read cases skip with code 77 on MSVC.
  - `Lifecycle.DiagnosticsTlsIndexReturnsWithTheSession` and `Lifecycle.DiagnosticsTlsIndexReturnsOnCacheShutdownWithoutASession` subscribe to both diagnostics dispatchers and require one shared index and tracked emits. The index must return exactly at Session teardown, or at `memory::shutdown_cache()` without a Session. In the Session case, a config callback owns the hook lifecycle subscription until `config::clear()`, so the release must follow that clear. A later subscription must then reserve a fresh index and record its frames.
  - `Lifecycle.DiagnosticsTlsIndexStaysThroughACacheRestart` restarts the cache inside a Session under a live subscription and requires no return and no leak record. A cache shutdown inside the Session after the drop must also leave the index to Session teardown.
  - `Lifecycle.DiagnosticsTlsIndexStaysWithALiveSubscription` keeps a subscription live across Session teardown. It requires one `Diagnostics` leak and a logged cause. A second teardown must not count the same index again, and a later cache shutdown must return it. A new kept ownership after that return must record again.
  - `Lifecycle.DiagnosticsTlsIndexWaitsForARunningHandler` parks a handler whose `reset()` removed it from the current list. Teardown must wait until the handler resumes, keep its frame tracked, and return the index without a leak.
  - `Lifecycle.DiagnosticsTlsIndexStaysWithAParkedHandler` keeps that handler parked past the one-second bound. Teardown must keep the index, record one leak, and log the cause. A later cache shutdown must return the index.
  - `Lifecycle.DiagnosticsTlsIndexTeardownInsideAHandlerNeverWaits`, `Lifecycle.DiagnosticsTlsIndexTeardownWithAnUntrackedEmitNeverWaits`, and `Lifecycle.DiagnosticsTlsIndexLoaderLockReleaseNeverWaits` drive a teardown inside a handler, while an untracked emit runs, and under a forced loader-lock probe. Each teardown must make no drain pause. Each kept ownership records one leak. The untracked case must log that cause. The loader-lock release must return an idle index without a callable destruction or a log line.
  - `Lifecycle.DiagnosticsTlsIndexProcessExitSkipsTheRelease` forces the process-exit context and requires the teardown to leave the ownership untouched.
  - `Lifecycle.DiagnosticsTlsIndexLoaderShutdownSkipsTheRelease` uses real process termination with a parked diagnostics emit and no published context. The fixture must return without a leak record.
  - `Lifecycle.GuardedReadDrainSkipsAtProcessExit` and `Lifecycle.GuardedReadLockSkipsAtProcessExit` terminate a thread with an active stripe or the VEH lock. The parent requires a clean exit and a fixture marker. Both cases skip on MSVC.
- `veh_unmap_dll.cpp` and `veh_survives_unmap.cpp` prove the MinGW handler lifetime through a verified unmap followed by native and C++ exceptions. `Lifecycle.GuardedReadHandlerRetiresAfterAHookOutlivesTheSession` drops the Session first and verifies fallback reads and closed region scans. `Lifecycle.GuardedReadHandlerReopensOnSessionRestart` and `Lifecycle.GuardedReadHandlerReopensOnCacheRestart` verify both reopen routes. `Lifecycle.GuardedReadHandlerRetiresWithTheSession`, `Lifecycle.GuardedReadHandlerRetiresOnCacheShutdownWithoutASession`, and `Lifecycle.GuardedReadHandlerRetiresWithTheInitializedCache` cover Session teardown, Sessionless cache shutdown, and the initialized cache. MSVC skips these handler proofs. The unknown-token control runs on both compilers.
- `vmt_late_static_owner.cpp` transfers a late-created `VmtHook` to a namespace-scope owner. The owner registers before the process VMT object gate exists. This shape differs from `Lifecycle.StaticHookCannotOutliveLedgerState`. Its exit code combines vptr, behavior, leak, and event checks. `HookTranslationUnitsRegisterNoExitDestructor` checks the gate in archive symbols. A destroyed winpthreads mutex silently reinitializes on the next lock, so a single-thread host cannot detect that defect.
...
- `staged_generation_dll.cpp` / `staged_generation_protocol.hpp` / `staged_generation_soak.cpp` pin the staged-generation reload pattern from the hot-reload guide. The generation DLL links a private DetourModKit archive and uses a test seam for XInput selection and an explicit wheel target thread. The host stages unique-name copies, rewrites each tag, loads each generation, and reads its instance-local status. The ten cases are:

  - `Lifecycle.StagedGenerationSoakReloadsWithFreshBytes` verifies 100 fresh logic images against one resident wheel host. Each live generation owns the sole lease. Shutdown releases it and reports zero logic pins and leaks. The loader probes the free lease, calls `FreeLibrary`, and requires address unmap before the successor load. The host snapshot stays at one hook, two thread handles, zero leases, and one mount generation.
  ...
  - `Lifecycle.StagedGenerationExistingCandidateUsesFreshName` places a file at the first staged name. The host must stage the generation under a fresh name, keep the existing file unchanged, and unmap the new generation.
  - `Lifecycle.StagedGenerationResourcesStayWithinBudget` verifies the [generation resource budget](../design/testing.md#generation-resource-proof).
  - `Lifecycle.StagedGenerationProofRejectsAnUnknownScenario` uses `WILL_FAIL` to reject an unknown token.

  The soak, local-wheel-retention, and uninstall-call-site cases use `SKIP_RETURN_CODE 77` because they require a window station. The resource case uses it only under sanitizers. If the synthetic XInput fixture is absent, the foreign XInput case fails. A cleanup fixture removes staged DLL and log files after all nine positive processes exit.
...
`xinput_detour_rundown.cpp` verifies clean reset, primary and ordinal routes, aliased exports, install rollback, and repeated uninstall. Each telemetry case checks the Input leak count and warning count. The warning probe reacquires the interception mutex, so a warning under that mutex deadlocks until the CTest timeout. Each case repeats uninstall without the owner. The repeated-uninstall case then renews the owner, so each repeat resets the reported chain again. The host drops its DLL reference before it resumes a parked caller. Retained routes preserve their module pins, while clean teardown permits DLL unload. `Lifecycle.LabelInventoryIsComplete` pins their `lifecycle-proof` registrations.
...
The `script-lint` label carries thirteen standalone Python self-tests. `HookExitDestructorCheckerSelfTest` covers the hook archive filter.

Positive and negative controls sit on the workflow boundary. `WorkflowTopologyIsBlocking` runs the topology gate against this repository's workflows. `ReleaseIdentityRefusesAMismatchedCandidate` requires a nonzero exit from a mismatched candidate. `SameBaseReplacementExecutionEvidence` appears below.

Python is a tests-ON configure prerequisite, so each configured test tree registers every script self-test. MinGW also registers two archive gates. `EmitPathHasNoEmulatedTls` rejects winpthreads thread identity from the input teardown owner path. `HookTranslationUnitsRegisterNoExitDestructor` rejects an exit-time destructor in the hook translation units. Both gates read symbols because source spelling does not settle either property.
...
- Lock-free structures use a test-only accessor. `test_event_dispatcher.cpp` defines `detail::EventDispatcherTestAccess` and proves the copy-on-write snapshot invariants. `EventDispatcherEmitOwnerRelease.*` uses it to prove the idle check of the teardown release: a running handler after `reset()`, `tombstone()`, `clear()`, or subscribe churn keeps the ownership. The installed class declares that unconditional friend. The installed definition carries no test macro. `scripts/check_header_hygiene.py` gates that contract. [docs/design/testing.md](../design/testing.md#test-seams-compile-out-of-shipping-builds) owns the rule.
...
- A live-instance counter exposes self-referential lifetimes. A detached `AsyncLogger` is kept alive by a retention root it holds on itself, so a retained writer is reachable only from itself. `test_logger.cpp` reads `DetourModKit::detail::g_async_logger_live_count_for_test` instead. The proofs are `DetachedWriterRetentionHasNoAllocationAndNoFiniteCeiling`, `CleanJoinBreaksTheRetentionRootAndLeaksNothing`, `PrePublicationFailureBreaksTheRetentionRoot`, `NonStandardThrowBeforePublicationIsContainedAndBreaksTheRoot`, and `PostPublicationThrowIsContainedAndKeepsThePublishedWriter`.
...
- `DetourModKit_bench_memory` (`bench_memory.cpp`) measures the cost of each way to read game memory from a hot path. The compared ways are the validation predicate (warm hit / cold miss), the direct fault-guarded read, and the pointer-chain primitives. It adds per-probe tail-latency and per-frame budget studies.
...
- Deliberate same-module splits: `test_memory_chain.cpp` (pointer-chain and plausibility primitives) is split from `test_memory.cpp`, and `test_string.cpp` (the `string::trim` cases) shares `format.hpp` with `test_format.cpp`. Scanner and RTTI have several focused suites for string xrefs, resolver tiers, parallel scans, reverse dissection, and heal scheduling. `test_unchecked_find_pattern_alloc.cpp` pins the per-call allocation count and the 1-based occurrence walk of `scan::unchecked::find_pattern`.
...
- Noexcept proofs: `test_noexcept_containment.cpp` pins the lock and move facts cited by the containment exception.
...
- Integration and lifecycle: `test_hook_integration.cpp` (cross-module hooks against the fixture DLL), `test_session.cpp` (`Session` / bootstrap / ordered `~Session` teardown / the module identity on both paths), and `test_mid_hook_context.cpp` ( `hook::MidContext` accessors).
- `SessionStart.FailedLoggerSetupRetiresThePublishedIdentity` proves identity cleanup after allocation failure. `SessionStart.UnexpectedLoggerFailureRetiresThePublishedIdentity` proves the other catch path. Both require a Stopped lifecycle.
- `SessionBootstrapTest.FailedOnReadyRequestsShutdownAndRetiresIdentity` requests shutdown from a failed callback. That request alone must clear the module identity and stop the lifecycle before the control-thread drain.
- `SessionTeardown.VmtHookOutlivingTheSessionRestoresItsObject` drops a `VmtHook` after its `Session`. It requires the original vptr, no leak record, and on MinGW no retained handler index.
- `SessionTeardown.RolledBackCacheStartKeepsTheRetiredHandlerEpoch` abandons a cache start before its Running publication after Session retirement. On MinGW, a later guarded read must not reserve a handler index.
- `SessionBootstrapReentrancy.BootstrapDrainCyclesRepeat` verifies MinGW handler readmission across bootstrap cycles.
...
- `[B-100]` boundary proofs: `test_memory.cpp`, `test_rtti.cpp`, `test_rtti_dissect.cpp`, `test_scan_resolve.cpp`, `test_anchor.cpp`, `test_manifest.cpp`, `test_sighealth.cpp`, `test_filesystem.cpp`, `test_profiler.cpp`, `test_diagnostics.cpp`, and `test_foundation.cpp` (the two `RegionLoaderBoundary` cases) each carry at least one `*LoaderBoundary` case. The case runs its module's loader-lock-safe entry points under `dmk_test::ForcedLoaderProbe` and measures the thread allocation delta with `dmk_test::thread_new_calls`. [`tests/fixtures/loader_lock_scope.hpp`](../../tests/fixtures/loader_lock_scope.hpp) owns the forced probe for suites in the main GoogleTest binary and for the `tls_index_return` host. The [loader-lock proof inventory](../design/lifecycle.md#loader-lock-proof-inventory) maps each proof family.
