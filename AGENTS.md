...
include/DetourModKit/    # Public headers -- one per module
  ...
  detail/                # Installed compile-visible support (DetourModKit::detail): pattern_core.hpp ONLY -- the constexpr heap/logger-free AOB parse-and-anchor core behind scan::Pattern. True-private impl (the SafetyHook bridge, the async-logger queue) lives in the non-installed src/internal/.
  ...
  diagnostics_dump.hpp   # One-call aggregator (Diagnostics::collect -> Diagnostics::Snapshot) bundling the intentional-leak counters and the heal-drift summary into one struct (the hook-population fields were dropped with the HookManager registry; a hook-population view is deferred to a later diagnostics fold)
...
- **Anchor registry (deferred):** the declarative anchor registry (`anchors.hpp` / `profile.hpp`) and its suites (`test_anchors.cpp` / `test_profile.cpp`) were removed in the v4 scan clean-break and are deferred to a later release; the registry backends are exercised through the scan-resolver, code-constant, string-xref, and RTTI suites above.
...
- **Diagnostics dump tests:** `tests/test_diagnostics_dump.cpp` covers `diagnostics_dump.hpp` (`Diagnostics::collect` -> `Snapshot`): empty inputs produce an all-zero snapshot, the leak counters fold in per subsystem and as a total, and the drift summary counts healed vs failed entries over a hand-built `DriftEntry` report. (The hook-population tally was dropped with the HookManager registry; it returns at a later diagnostics fold.)
