...
- `evidence` - the literal bytes of the span the winning byte-pattern rung matched, captured during that match. It is the content counterpart to `image`, which is layout-only. An in-place code patch that leaves the PE headers equal moves this and nothing else. Present only when a `RipGlobal` anchor wins on a `Direct` or RIP-relative rung, the one path that returns the span it matched. For a RIP-relative winner, semantic decode, disp32, and target use an immutable instruction snapshot captured in the same guarded sweep, so a post-sweep mutation cannot split the returned target from this evidence. Evidence is absent when a rung wins through a structure instead (RTTI vtable, string xref), and absent for the `VtableIdentity`, `StringXref`, `ExportName`, `Manual`, and `Quorum` kinds. It is also absent for `CodeOperand`, which locates through a byte pattern but returns a decoded `Scalar` rather than the site.
