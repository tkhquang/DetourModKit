...
std::string DetourModKit::Filesystem::get_runtime_directory()
{
    ...
    Logger &logger = Logger::get_instance(); // Get logger instance for messages

    try
    {
        // Get a handle to the current module (DLL or EXE).
        // The address of this function (get_runtime_directory) is used to find the module.
        if (!GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT,
                                (LPCSTR)&get_runtime_directory, // Address within the current module
                                &h_self_module) ||
            h_self_module == NULL)
        {
