 ...
 * @brief String-reference (xref) anchor backend: locate an immutable string literal in a module image, then resolve the
 *        unique instruction that references it.
 *
 * Two fail-closed phases. Phase 1 locates the single occurrence of the query string in the image's readable pages
 * (reusing the scanner's module-scoped readable scan). Phase 2 finds the single RIP-relative reference to that string:
 * a fast, desync-immune shape scan for the dominant lea/mov forms by default, or that same shape scan plus a
 * Zydis-verified linear sweep (broad_match) that also recognizes the rarer shapes (cmp/push [rip+d], no-REX lea/mov,
 * ...).
 *
 ...
 * consumer links DetourModKit (which already links Zydis statically) without ever needing Zydis headers on its own
 * include path. Keeping the broad sweep here also keeps scanner.cpp -- the core scan engine -- free of the decoder.
 */
...
namespace DetourModKit
{
    namespace
    {
        ...
        // Builds a literal-byte CompiledPattern from a string query: the raw bytes of the text (UTF-8, or each code
        // unit widened to UTF-16LE) plus an optional trailing NUL. The query content is emitted as a hex AOB and run
        // through parse_aob so the string scan reuses the exact same compiled-pattern path as every other AOB. Returns
        // nullopt on an empty query. Non-ASCII UTF-16 is out of scope: each byte is widened verbatim (Latin-1), which
        // covers the ASCII identifiers that anchor strings almost always are.
        ...
        // displacement. That required REX.W byte makes the instruction exactly 7 bytes and the disp32 sits at offset 3,
        // so the candidate is self-delimiting from its shape: this needs no instruction-aligned linear sweep and
        // therefore cannot desync on data or jump tables embedded in .text.
        //
        // The resolved target is next-instruction-address + sign-extended disp32, done in unsigned modular arithmetic
        // so it is well-defined for every input. Only a target that exactly equals string_addr is accepted, which is an
        // extremely strong filter: string_addr is the already-located, plausible
        // .rdata address, so this equality subsumes the plausible_userspace_ptr floor (an implausible target cannot
        ...
        // RIP-relative reference shapes the narrow
        // scan does not model -- cmp [rip+d], imm; push [rip+d]; a no-REX lea/mov;
        // any instruction whose memory operand is [rip+disp] and resolves to string_addr. The caller merges this with
        // the narrow scan so broad_match cannot lose coverage for the default lea/mov anchors.
        //
        // The sweep decodes each position with the same decoder code_constant.cpp uses (ZydisDecoderDecodeFull +
        // ZydisCalcAbsoluteAddress). x86-64 is not self-synchronizing, so on a decode failure the cursor advances one
        // byte to realign past embedded data or jump tables (recovery); on success it advances by the decoded
        // instruction length, which is always >= 1 so the sweep cannot stall. As in the narrow scan, only a
        // RIP-relative operand whose absolute target exactly equals string_addr counts. That exact-target filter
        // subsumes the plausibility floor and neutralizes a mid-.text desync:
        // a bogus instruction decoded out of embedded data almost never carries a RIP operand resolving to precisely
        // string_addr. The one residual miss is a reference truncated at the very end of a window (too few bytes to
        // decode, so the sweep byte-restarts past it); that cannot happen mid-image, since a
        // PE's executable section is one contiguous window and a real reference never sits in its final few bytes.
        // Counting stops at the second referencing instruction so the caller fails closed on ambiguity.
        ...
        // Best-effort enclosing-function entry for a referencing instruction. Walks backward for the nearest function
        // boundary -- a terminal RET (0xC3) or a run of INT3 (0xCC) alignment padding -- and returns the first byte
        // after it that passes is_likely_function_prologue, skipping any further INT3 padding. The back-scan is bounded
        // so a pathological region cannot scan unboundedly, and it fails closed (returns 0) when no boundary is found
        // in the window. This is a heuristic, not control-flow analysis: a 0xC3 / 0xCC byte that is actually operand
        // data inside an instruction can mark a false boundary, which is why the default return mode is the exact
    ...
    std::expected<std::uintptr_t, Scanner::StringXrefError> Scanner::find_string_xref(const StringRefQuery &query,
                                                                                      Memory::ModuleRange range)
    {
        ...
        // Phase 1: locate the single occurrence of the string in the image's readable pages. The linker pools identical
        // literals, so a second occurrence makes the anchor ambiguous and must fail closed.
