...
namespace DetourModKit::scan
{
    namespace
    {
        ...
        // Build the engine's heap-backed CompiledPattern from a v4 value Pattern. anchor_index is the position the scan
        // should prefilter on, already translated to the engine's sentinel convention (anchor == size() means "no
        // fully-known byte", which the engine handles with a degenerate masked compare). Allocates two small vectors,
        ...
        // disp32 the instruction spans under a fault guard and computes (next-IP + sign-extended disp). Both screen the
        // result through the plausible-userspace floor, so a faulted read or a crafted displacement is a miss, never a
        // hit at a near-null or kernel-range address (matching resolve_rip_relative's contract).
                                                                           ...
                                                                           const Candidate &candidate) noexcept
        {
            ...
            const std::uintptr_t displacement_address = match + static_cast<std::uintptr_t>(candidate.displacement());
            const std::optional<std::int32_t> displacement = Memory::seh_read<std::int32_t>(displacement_address);
            if (!displacement)
            {
                return std::nullopt;
            }
            const std::uintptr_t next_instruction = match + static_cast<std::uintptr_t>(candidate.instruction_length());
                                                                       ...
                                                                       Memory::ModuleRange range, bool &applicable)
        {
            ...
            // faulted region mid-scan makes the count a lower bound, so a single hit over an incomplete sweep does not
            // prove uniqueness; fail closed. More than one hit makes the rebuilt jump ambiguous; fail closed.
            ...
            return resolve_byte_candidate(anchored, candidate);
        }
        ...
        // Run hooked-prologue recovery across the ordered ladder: for each Direct candidate, try each shape until one
        // uniquely recovers an executable target. Reproduces the v3 scan_candidates_hooked_prologue control flow
        // without the Logger coupling.
                                 ...
                                 std::span<std::size_t> out) noexcept
    {
        ...
        // UniqueFirst: three stable passes over the declared order. Every candidate falls into exactly one pass, so the
        // result is a permutation of [0, count).
    ...
    ScanRequest borrow(std::span<const Candidate> ladder, std::string_view label, Region scope, bool prologue_fallback,
                       bool require_unique, CandidateOrder order) noexcept
    {
        return ScanRequest{
            .ladder = ladder,
            .label = label,
    ...
    Result<Hit> resolve(const ScanRequest &request)
    {
        ...
        // Lay out the try order once. The haystack histogram is sampled lazily on the first byte candidate and shared
        // across every byte candidate in the ladder, since they all scan the same scope.
        ...
        for (std::size_t k = 0; k < ordered_count; ++k)
        {
            ...
            if (incomplete)
            {
                // A faulted region makes the occurrence count a lower bound. A hidden earlier match or duplicate could
                // live in skipped bytes, so accepting this candidate would turn a race into a wrong address.
                continue;
            }
            if (ambiguous)
            {
                // Ambiguous in scope: the lowest-address match is not provably the intended target, so fall through to
                // the next candidate rather than commit to an arbitrary site.
                continue;
            }
        ...
        if (request.prologue_fallback)
        {
            const FallbackOutcome fallback =
                resolve_prologue_fallback(request, std::span<const std::size_t>{order.data(), ordered_count}, range);
            if (fallback.hit)
            {
                return *fallback.hit;
            }
            ...
            {
                // A Direct candidate existed to rebuild, but its literal tail was too short for any shape. This is a
                // distinct diagnostic from a plain miss (a name/string/RipRelative-only ladder has no Direct row).
    ...
    const std::byte *unchecked::find_pattern(Region region, const Pattern &pattern) noexcept
    {
        ...
        try
        {
            // The raw primitive does no page filtering, so the caller owns readability; it also does not consult the
            // haystack histogram (that override accelerates the page-gated scan), using the Pattern's compile-time
            // anchor directly.
