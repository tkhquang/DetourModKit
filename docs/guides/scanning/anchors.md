...
Before resolving, the members are checked for independence pairwise: corroboration only means something if every signal is separate evidence. Reusing one `Anchor`, pairing two `Manual` literals, sharing any candidate rung, selecting two operands from one instruction, or spelling the same effective export as both scoped and explicit makes the quorum `QuorumNotIndependent`. The comparison uses evidence content rather than view storage, ignores cosmetic candidate names and operand selectors, and compares ladder atoms as sets so a shared fallback is enough to establish one failure domain.
