...
A raw proof's failure exits are as load-bearing as its success path, and they are where the poll thread is still live. Returning from a scope a binding callback captured by reference destroys that callback's payload underneath it, and clearing a process-wide seam the loop is calling destroys a callable mid-call. Either turns an intended red diagnostic into a native access violation or a hang, which reports as a different failure than the one the proof states.

The order is therefore: unblock parked callback work, run the engine down and join, and only then clear the seams. [`tests/lifecycle/input_seam_cleanup.hpp`](../../tests/lifecycle/input_seam_cleanup.hpp) owns it as `dmk_lifecycle::InputSeamOwner`, declared immediately after the engine starts and after every local a callback captures, so reverse destruction order makes the sequence structural rather than something each exit path has to remember. Two details are not optional. A callback parked on another callback's flag never returns, so a join without the unblock wedges instead of failing; and `Input::shutdown()` reached from a binding callback hands its rundown to the process reaper and returns before it has been delivered, so a host that can self-shutdown must supply a completion predicate or it will clear seams while a callback can still run. `dismiss()` exists for the one shape that genuinely wants the engine left running -- a loop iteration re-registering against a still-started manager -- and is valid only where the callbacks have completed and nothing is parked.

Three bounded negative controls drive that path on purpose rather than waiting for a red premise to find it: `InputLifecycleProof.AbandonedParkedCallbackRunsDownBeforeClearing`, `AbandonedFacadeDrivePremiseRunsDownBeforeClearing`, and `AbandonedSelfShutdownPremiseRunsDownBeforeClearing`. Each starts the real engine, parks a callback inside the poll thread, abandons its premise, and then asserts what the rundown did with it. The ordering oracle is read from inside the parked callback: after a window a correct rundown cannot use -- it is blocked joining that very body -- the callback records whether its seam is still installed. A rundown that cleared before joining loses that race and is named doing it; one that never unblocked hangs into the case's `TEST_TIMEOUT`; one that did not run at all leaves the engine started. Exit zero requires released, still-installed, stopped, then cleared, in that order.
...
[`scripts/run_lifecycle_soak.py`](../../scripts/run_lifecycle_soak.py) is the final-candidate lifecycle gate used by both Release jobs. It inventories CTest first so a missing label or named regression cannot vacuously pass -- every entry of `REQUIRED_SOAK_PROOFS` must appear exactly once. That set covers the three staged-release harness cases whose subject is the repetition (including the exact in-place-rebind case), the forced-cleanup control, five CMake-owned input raw proofs, the three bounded cleanup controls described above, and four exact XInput pair-health raw proofs; an omitted registration would otherwise disappear from the inventory rather than fail. `scripts/test_run_lifecycle_soak.py` carries an independently spelled expected tuple, rejects drift from the production tuple, then drops each required name in turn and requires the gate to fail and name the dropped proof, so the list cannot rot into a group count with a hole in it. The soak temporarily arms per-executable WER LocalDumps only for the proof processes and drives `fast_fail_probe wer-crash` through a bounded wait to require a complete `MDMP` control before trusting capture. The assertion is that the control terminated with `STATUS_FAIL_FAST_EXCEPTION` (`0xC0000602`), not merely that it was nonzero, because only that status proves the control exercised the path WER is being asked to capture. Python is what makes that assertion expressible: it reports the raw process exit status, whereas a POSIX shell on Windows maps a fatal status onto a signal number and loses the distinction. `winreg` arms and restores the machine-wide WER values, and the completed-dump check opens with no sharing through `CreateFileW`, so a dump WER is still writing reads as incomplete rather than being trusted for its first four bytes. The control dump is deleted. The script then repeats every `InputLifecycleProof` 200 times serially, runs `Lifecycle.FullLifecycleExit` 100 times serially through `run_lifecycle_proofs.sh`, and runs all `lifecycle-proof` cases 20 times at parallelism four, stopping on the first failure. A restoration guard covers arming and every proof: each newly created key is ledgered before another registry operation can fail, every captured value and key is isolated so one failure does not abandon later work, and an otherwise successful soak exits nonzero unless the registry returns to its pre-run state. A key that was absent before the run is deleted only when it is empty; unowned content is left intact and reported as restoration residue. An earlier soak or unexpected failure stays primary while every cleanup failure is printed alongside it, and the success message is emitted only after restoration passes. `scripts/test_run_lifecycle_soak.py` drives those paths against a stateful stub registry -- so the failure modes are provable without breaking a real machine -- and is registered as the `LifecycleSoakRestorationSelfTest` script-lint ctest. A real crash minidump remains for the workflow's short-retention diagnostic artifact. WER captures unhandled crashes, not hangs, CTest timeouts, cancellations, or ordinary nonzero exits, so CTest's per-case timeouts and `LastTest.log` remain the evidence for those failures. Minidumps can still contain sensitive process memory and must never become release assets. Because WER policy is machine-wide, do not overlap two soak-script invocations on one host; the checked-in Release jobs use separate hosted VMs.

### Script self-tests (`scripts/`, Python)

The `script-lint` label carries eleven standalone Python self-tests: `HeaderHygieneStripperSelfTest`, `EmitTlsCheckerSelfTest`, `ExportEqualityGateSelfTest`, `MechanicalStyleCheckerSelfTest`, `TestLabelInventoryGateSelfTest`, `LifecycleSoakRestorationSelfTest`, `RuntimeDirResolverSelfTest`, `BenchmarkResultsCheckerSelfTest`, `WorkflowTopologyCheckerSelfTest`, `ReleaseIdentityCheckerSelfTest`, and `GTestExecutionCheckerSelfTest`, plus two positive/negative controls on the workflow boundary -- `WorkflowTopologyIsBlocking`, which runs the topology gate against this repository's own workflows, and `ReleaseIdentityRefusesAMismatchedCandidate`, which runs the real identity command line on a mismatched pair and requires a nonzero exit -- and `SameBaseReplacementExecutionEvidence` (below). Python is a tests-ON configure prerequisite because the RTTI generation fixture needs it, so every configured test tree registers all of them; MinGW also registers the `EmitPathHasNoEmulatedTls` archive gate, which rejects winpthreads thread identity from the input binding teardown owner path.

#### The canonical workflow contract

[`scripts/workflow_contract.py`](../../scripts/workflow_contract.py) holds the reviewed shape of all seven workflows as data, outside the parser that reads them: triggers, jobs, runners, dependencies, the complete ordered step list of every job, each step's shell, reviewed condition and material environment, and the exact ordered program of every step whose text is load-bearing. A normalized-source SHA-256 identity closes the rest of the YAML surface, including every action reference and input, trigger value, strategy and matrix, permission, default, output, comment, and otherwise-unlisted run body. [`scripts/check_workflow_topology.py`](../../scripts/check_workflow_topology.py) compares against both layers rather than searching workflows for fragments. The reader requires valid UTF-8 and normalizes CRLF or bare CR to LF before hashing, so checkout line endings do not change the identity; every other source change requires the workflow and its reviewed digest to change together.

The distinction is the whole boundary. A presence check answers "is the reviewed line still here?", and the answer stays yes after an assignment is inserted that shadows the value the line reads, after the single success exit is relocated to the top of a guard, after an action or matrix is retargeted, and after a wrapper shell rewrites the status the guard computed. A comparison answers "is this the reviewed workflow?", which none of those can answer yes to, because each changes the exact source or a structured entry. Every structured entry is bidirectional: an unreviewed job, step, shell, environment or condition is refused, and a reviewed one that was deleted or renamed is refused. A workflow change therefore requires its contract change in the same review; reviewers inspect the workflow diff before accepting the corresponding digest update.

Identity contexts enter the shell only as quoted data. Each workflow maps the dispatch input, event commit and publication ref into the exact step environment the contract records, then one exactly reviewed command expands each value once inside quotes. Shell-significant context text is therefore not re-evaluated as Bash source, and the source identity refuses workflow/job startup hooks or any line that could rewrite the bindings. [`scripts/check_release_identity.py`](../../scripts/check_release_identity.py) compares those arguments, resolves the checkout with inherited `GIT_*` repository-selection variables removed, and optionally restricts publication to one exact ref. `release.yml` runs it with `--verify-checkout` in both `validate-version` and `create-release`; the second guard also requires `refs/heads/main`, so the publish boundary re-decides identity instead of trusting an earlier job. `arch-gate.yml`, `sanitizers.yml`, `simd-tier-correctness.yml`, and `coverage-pages.yml` run the same helper on manual dispatch, so evidence recorded at a final candidate says which commit it describes; each declares `expected_sha` as a required string input, and the checker refuses a candidate guard whose input went missing or stopped being required.

A required step is unconditional. A step that does not run cannot fail, so a step-level `if` on a required command is a guard reporting green without having run it. The exception is the exact condition recorded on that step in the contract: those either strengthen the step (`!cancelled()`, so a later gate still reports after an earlier failure), guard a diagnostics upload, bind a leg to a manual dispatch, or select a matrix route, and none can skip a guard on an otherwise passing run. Conditions are matched by exact value in both directions, so a reviewed step cannot keep its name and take a false condition instead, and a condition the contract does not record cannot appear at all.

Exit status must reach the job. `|| true` was never the only way to discard a failure -- any fallback that exits zero does it, and `|| echo ignored` is exactly as green -- so `||` is refused by construction rather than by enumerating the fallbacks somebody might reach for. It is refused on the raw line as well as on the token stream, because an operator inside quoting or a command substitution is still evaluated by the shell while a token scan sees one opaque word. The one carve-out decides nothing: shell test compounds joined inside a conditional head, where every operand is a `[`/`[[` test rather than the work the step exists to run. The same rule covers the other ways a failure stops propagating: `set +e`, `set +o errexit` and `set +o pipefail`; `eval`, which hides the program whose status is being checked; a CMD `&` chain or pipeline, which reports the last command instead of the failing one; a negated command; and a PowerShell `Write-Warning`, which leaves a detected breakage green, so both producers' empty-install-prefix check uses `Write-Error` and a nonzero exit.

Shells are an allowlist, not free text. Only `bash`, `cmd`, `pwsh`, and `powershell` may be declared, and each step must declare the one the contract records for it. A custom template such as `shell: bash {0} || true` is not a shell choice but a status rewrite applied to every command in the step without touching a line the reviewer read, and `sh` is excluded because GitHub runs it without pipefail. Each step is then read under that shell, which decides both which status rules apply and which continuation marker joins physical lines. The marker is not cosmetic: bash removes a backslash-newline and PowerShell a backtick-newline without inserting whitespace, so a command name can be split as `Write-` plus `Warning`, and reading a PowerShell step with bash's marker would leave the halves in separate logical lines and match neither. Three steps whose bodies a POSIX tokenizer cannot read -- two PowerShell blocks whose strings end in a backslash, and one embedded `python -c` script -- carry exact program pins instead, which is the stronger of the two boundaries.

Triggers are checked too. The PR Check, Architecture, and Quality contexts are the ones a repository ruleset is meant to require, so their `pull_request` triggers must stay path-unconditional: a `paths` or `paths-ignore` filter leaves a pull request waiting forever for a context GitHub never creates. Only `coverage-pages.yml`'s push route may carry a filter, because it republishes what `main` already is. A workflow file that no contract entry names is refused outright as an unreviewed route into the repository. A secret expression is refused wherever it appears outside release.yml's audited publish job, including `toJSON(secrets)`, which names no single secret and expands to all of them; a credential on a pull-request-triggered route is the more dangerous placement, because that route runs on a contributor's head.

[`scripts/test_check_workflow_topology.py`](../../scripts/test_check_workflow_topology.py) has 126 tests, and [`scripts/test_check_release_identity.py`](../../scripts/test_check_release_identity.py) has 22. The matrix challenges the structured diagnostics and the exact source boundary. It mutates pinned programs by insertion, deletion, reordering and respelling -- including shadowing, `export`, `unset`, the tag step's single success exit relocated to the front, dropped checkout/ref controls, direct context interpolation, unquoted or retargeted environment values, a comment spliced between a continued line and its continuation, a reintroduced retry, and one unreviewed line inserted into each of the twenty-eight named pinned steps, which are held equal to the set of contract steps carrying a program so neither side can drift. Focused status mutations must still be refused by their unsafe construct. Exact-source mutations cover every workflow, action replacements, trigger values and flow mappings, deleted matrix legs, job defaults, unlisted program replacement or last-command masking, nested substitutions, shell-startup redirects, credential references in every workflow rather than only the release route, line-ending portability, and invalid UTF-8. Around those sit deleted, renamed, added and reordered jobs and steps; retargeted conditions; unreviewed shells; path-filtered contexts; dispatch-input, dependency, publish-mode, credential, clang-tidy and SIMD controls. The identity self-test drives the real command line in a subprocess for malformed or newline-tainted SHA values, wrong event/ref/checkout, hostile `GIT_DIR`, Git failure, and exact positive identity including the publish checkout. Every negative asserts the refusal message, not only the exit code, and process-boundary controls prove both CLI exit statuses.

Two distinctions in that policy are deliberate. The three exact `TEST_MAJOR`/`TEST_MINOR`/`TEST_PATCH` captures may end in `|| true`: their failure becomes an empty value that the following validation rejects with a useful diagnostic. Other captured or bare success fallbacks are refused. And a zero exit is allowed only in the exact reviewed tag step after the existing annotated tag has been proved to resolve to the candidate; an early success anywhere else is refused.

[`scripts/test_resolve_runtime_dir.py`](../../scripts/test_resolve_runtime_dir.py) pins the compiler-runtime resolution the three proof wrappers share: that a preset tree's bare `g++` cache entry still resolves to the real MinGW `bin` directory, that an MSVC tree resolves to nothing, that a bare compiler name is refused rather than silently resolving to `.` (which is what made the shell wrappers prepend the repository root), and that the separator is forward slashes on every interpreter.

### Executed-case evidence (`scripts/check_gtest_execution.py`)

A failing case is red and a case that never ran is green, so a host-conditional proof cannot be read out of a pass/fail count. `MemoryTest.ModuleRangeFor_CompletedSameBaseReplacementReportsTheReplacementExtent` calls `GTEST_SKIP()` when the loader will not place both fixture variants at the reserved base, which is honest on a developer machine and unacceptable in a release candidate. The case therefore publishes `RecordProperty("dmk_same_base_replacement", "executed")` as its last statement, after both mapping steps and the replacement-extent observation have succeeded; every earlier exit is a skip or a fatal assertion, both of which leave the property absent.

`tests/CMakeLists.txt` registers two tests around that. `SameBaseReplacementExecutionRecord` runs the exact case with `--gtest_output=xml:` into the build tree (a CTest fixture, so ordering is not a guess), and `SameBaseReplacementExecutionEvidence` reads the record back through [`scripts/check_gtest_execution.py`](../../scripts/check_gtest_execution.py). Locally the checker is given `--skip-exit-code 77`, so an unqualified host reports Skipped rather than a false pass; both Release producers in `release.yml` run the same checker over their own tree's record **without** it, where an unavailable exact-base mapping is a red candidate host. The checker refuses a missing, unreadable or unparsable report, an absent or duplicated case, a case that states neither a status nor a result, a not-run/suppressed/skipped case, a failed case, a missing property, and a wrong property value, and `--skip-exit-code` never launders anything but a genuine skip: a case that records a failure or an error is not a skip whatever else the report claims about it. [`scripts/test_check_gtest_execution.py`](../../scripts/test_check_gtest_execution.py) (20 tests) feeds it each of those shapes plus a foreign suite carrying the same case name, a name differing only in letter case, and the real CLI exit codes.

### Backend equality (`scripts/check_backend_patch.py`)

The vendored patch is only half the backend contract: the submodule working tree is where it is applied, so source-capable content left there can enter the build too. Exactly two source states are legitimate -- the pristine pinned base before any configure, and exactly the reviewed patch output after one. The checker rejects staged content, assume-unchanged or skip-worktree flags, ordinary untracked content other than the configure-time `.dmk_patch.lock`, ignored content outside a frozen allowlist of SafetyHook's non-source build/IDE/generated-output roots, a tracked edit outside the patch's file set, an incomplete patch, a reconstructed-byte mismatch, or any failed state query. `--expect-state pristine|patched` pins which state a caller requires; the blocking `backend-patch` quality job proves the fresh checkout is pristine, applies the patch through `cmake -P cmake/DMKBackendPatch.cmake`, and proves the result is exactly the reviewed output. The CMake helper enforces the same boundary at configure time, immediately after applying the patch and still under its lock, and script mode rejects missing directories.

That boundary is byte equality, not a path set. An edit inside a target the patch already owns leaves the changed-path set identical, and once the patch's own lines are present the idempotence reverse-apply stays clean too, so both of the older signals report green while configure compiles bytes nobody reviewed. Both implementations require the exact pinned submodule commit, refuse local replacement-object redirects, and reconstruct with replacement objects disabled. `cmake/DMKBackendPatch.cmake` rebuilds each owned target from the pinned commit's blob plus the patch, in a scratch tree anchored as its own git worktree root inside the submodule's git directory -- never the live checkout, and never anywhere the enumeration of backend state can see -- and compares. Normalization is explicit on both sides of one shared model: `lf()` in Python, `git diff --ignore-cr-at-eol` in CMake, because one has Python available and the other has only git. Both reconstruct from the pinned commit itself rather than from whatever `HEAD` resolves to, so a checkout that silently moved cannot supply its own bytes as the expected answer. `scripts/test_check_backend_patch.py` has 36 cases, including direct positive and contaminated-fixture executions of the production CMake entry point, a same-target drift whose fixture is deliberately large enough that the smuggled bytes fall outside every hunk (a one-line fixture cannot express the defect, because any added byte lands in the patch's own context and the reverse-apply catches it first), alternate committed-base and replacement-object refusals in both models, an agreement matrix asserting the two implementations accept and refuse the same states, and the `CMP0057` declaration that `cmake -P` needs because script mode carries no project policy state.

### Build-tree consumer evidence

`tests/package_build_tree` is the `add_subdirectory` half of the packaging contract, as `tests/package_smoke` is the `find_package` half. Both Release producers in `release.yml` configure, build, and run it at the Release configuration they package, and `scripts/check_workflow_topology.py` refuses a producer that stops doing so -- a producer that dropped it would still upload an archive, and the missing proof would be visible only as an absent log line in a job that reported success.
...
- **Poll-loop `std::function` seams have a lifetime rule, and it is a proof of its own.** The poll loop reads `g_input_post_stage_probe` and its siblings without synchronization on every cycle, so a case that installs one must install it while the poller is STOPPED, must arm the phase it parks on through a separate atomic afterwards, and must never clear it until the poll thread is joined. The probe's captures are the case's own barrier atomics, so they are declared before the poller and the cleanup owner is declared after it: on every exit, a fatal assertion included, the owner opens any parked dispatch, joins, and only then destroys the probe. Assigning the global with the thread running, or clearing it before the join, writes over a `std::function` the poll loop is executing and frees a parked frame's barriers, which is an intermittent native access violation rather than a test failure. `InputLifecycleProof.StagedProbeCleanupJoinsBeforeDestroyingProbeCaptures` is the bounded forced-cleanup control: it parks the poll thread, leaves the scope without releasing it, and requires the probe's captures to have been destroyed only after the join. Reordering the owner to clear before joining fails that case deterministically, and it is the shared owner used by all three staged-release proofs and the typed-drain case.
...
[`scripts/check_benchmark_results.py`](../../scripts/check_benchmark_results.py) consumes the captures and refuses missing, malformed, non-finite, duplicated, spliced, unclosed, or internally contradictory evidence; failed deterministic gates in either current or baseline captures; mismatched deterministic scenarios; missing/wrong build or tier roles; and missing/mismatched stable-host identity. Every record is identified by an exact (suite, name) pair whose suite is nonempty and dot-free and whose name is the suite plus `.` plus a nonempty remainder. `--require` binds that pair and requires a deterministic gate, while `--metric-ratio` uses the same tuple for current and baseline metrics; neither can be satisfied by another suite. `tests/bench_gate.hpp` enforces the same identity shape, rejects a ledger that emits no gate, and makes close terminal. The standalone `dmk_bench_gate_probe` compiles that header as its own producer and exercises valid, refused, failed, zero-count, duplicate-close, and post-close modes. Timing gates and valid metric comparisons are reported everywhere and enforced only under `--stable-host`, because a shared runner cannot tell a regression from a noisy neighbour. `scripts/test_check_benchmark_results.py` has 57 parser, policy, provenance, CLI, and compiled-producer cases and requires the probe path from CTest or the release workflow. It resolves that path to an absolute one before executing it, because Windows refuses a relative program path spelled with forward slashes and no leading `./` -- which is exactly what the release workflow's `find` hands it, while CTest's generator expression hands it an absolute one; one case pins that spelling so the two callers cannot diverge again.
