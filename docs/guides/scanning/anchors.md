...
`quorum_members` are non-owning pointers into your own anchor storage, so keep the members alive across the `resolve` call. `quorum_threshold` is `N`: `0` (the default) means unanimous, so a two-member quorum with the default is the strict 2-of-2 corroboration; a quorum is corroboration, so an explicit `N` below 2 or above the member count is malformed. `QuorumMatch::ExactValue` (the default) requires identical values; `QuorumMatch::WithinTolerance` accepts a gap no larger than `quorum_tolerance` (a negative tolerance never accepts). A single qualifying cluster commits its smallest qualifying center, independent of declaration order. If two qualifying centers disagree, resolution returns `AnchorStatus::QuorumAmbiguous` instead of choosing one by order. Fewer than `N` agreeing members and malformed declarations return `AnchorStatus::Failed`.

Independence is checked twice because corroboration only means something when every vote is separate evidence. Before resolution, reusing one `Anchor`, pairing two `Manual` literals, sharing any candidate rung, selecting two operands from one declared instruction, or spelling the same effective export as both scoped and explicit makes the quorum `QuorumNotIndependent`; this comparison uses evidence content rather than view storage, ignores cosmetic names and operand selectors, and compares ladder atoms as sets. After resolution, the winning physical sources are compared too: overlapping byte-pattern spans, or a decoded instruction site overlapping another winner's span, are one failure domain even when different patterns, bounded jumps, result markers, or walk-backs selected them. Exports are correlated by the entry they actually resolved through rather than by the name that asked for it, so two names that share an `AddressOfFunctions` slot, or that carry the same function RVA in one image, count once -- which is what a declaration alone cannot decide, since which names alias is a property of the live export table. Equal resolved values alone are not dependency, so genuinely distinct instructions that reference the same target remain independent, and an export still corroborates against a signal read from somewhere else.
