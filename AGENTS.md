...
include/DetourModKit/    # Public headers -- one per module
  ...
  anchors.hpp            # Declarative self-healing anchor registry over the vtable / cascade / code-constant / string-xref / manual backends, plus two-signal quorum corroboration (with sub-anchor independence checks), optional post-resolve validators and opt-in validator policies, opt-in parallel table resolution, and a manifest quality diagnostic (Anchors::resolve, resolve_all, resolve_all_parallel, assess_quality)
