...
{
    // Resolve a RIP-relative instruction whose computed target lands below USERSPACE_PTR_MIN (0x10000). The
    // displacement bytes are read from committed memory, but the resolved address is not a plausible user-mode pointer,
    // so the resolver must fail closed with ImplausibleTarget rather than return a near-null address that faults on
    // first dereference.
    //
...
namespace
{
    ...
    // are distinct for any run shorter than 256. marker_index, when non-negative, inserts a `|` offset token before
    // that byte.
...
{
    // A guarded read-only page reads as PAGE_READONLY | PAGE_GUARD; the guard modifier must exclude it from the
    // readable sweep, otherwise the first touch raises STATUS_GUARD_PAGE_VIOLATION.
...
// overwritten by SafetyHook or MinHook starts with a JMP rel32 (0xE9); the helper must still accept it so the resolver
// path stays usable when a sibling mod hooked the same function first.
...
// allocators that need a full 64-bit reach. Only the first byte (0xFF) is examined by the helper, but the test seeds
// both bytes so the buffer matches what a real patched prologue would look like.
