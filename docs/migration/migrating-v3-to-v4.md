...
- `HookConfig::fail_if_already_hooked` -> `hook::Options::fail_if_already_hooked`. Its refusal is two mechanisms with two codes: `ErrorCode::TargetAlreadyHookedByThisKit` when this linked DMK archive already holds a record for the address (a hook created but not yet armed counts, so the prologue is not necessarily patched), and `ErrorCode::TargetAlreadyHookedByAnotherModule` when the ledger has no record and the prologue decode finds a branch out of the target's own module. The correct response differs -- drop your own handle, versus coexist with or abandon a target someone else owns -- so branch on the code rather than on a single already-hooked value. A record a pin left behind (`Hook::release()`, or a teardown that could not restore) belongs to no handle, so there the first code has no handle to drop and refuses every later strict install on that target for good. v3's `HookError::TargetAlreadyHookedInProcess` and the pre-split v4 `ErrorCode::TargetAlreadyHookedInProcess` have no direct replacement; there is no compatibility alias, and matching on either name fails to compile. Splitting the enumerator renumbers every hook code declared after it, so a consumer that persists or transmits raw `ErrorCode` values must re-read them rather than compare across versions.
- `diagnostics::total_intentional_leaks()` now counts the caller-requested leaks too, not only DMK's defensive pins: `Hook::release()` and `VmtHook::release()` each book one event. Absolute totals therefore step up against v3 and against earlier v4 builds; compare deltas across an operation instead.
...
- `get_hook_counts` / `get_hook_ids` -- gone. Aggregate population figures are now read-only via `diagnostics::collect().hooks_total` / `hooks_active` / `hooks_disabled`. Each live hook counts on its own, so two hooks that share a name (on distinct targets) both appear in the totals.
...
Physical trust is stricter at the v4 boundary. A quorum returns `QuorumNotIndependent` when differently authored byte selectors resolve through overlapping physical spans, even if their declarations differ; genuinely distinct instruction sites may still agree on one value. A `rip_relative` candidate or manifest rung must include its complete disp32 in the matched suffix from the `|` marker, and target decode uses the sweep-time snapshot rather than rereading live bytes. That snapshot is clamped to the scope you declare, not to the owning module, so a rung whose declared `instruction_length` runs past the end of a narrow scope now reports `NoMatch` where the v3 live reread resolved it; widen the scope to cover the whole instruction. `CodeConstant::byte_width` and its anchor/manifest equivalents accept only 0 or 1 through 8; older out-of-domain values now fail before scanning instead of behaving as full width.
