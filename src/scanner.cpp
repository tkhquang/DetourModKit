...
namespace DetourModKit
{
    namespace
    {
        ...
        std::optional<size_t> verify_pattern_avx2(const std::byte *pattern_start,
                                                  const Scanner::CompiledPattern &pattern, size_t start_offset) noexcept
        {
        ...
        DMK_AVX512_TARGET
        DMK_NO_SANITIZE_ADDRESS
        std::optional<size_t> verify_pattern_avx512(const std::byte *pattern_start,
         ...
         * @details Higher scores indicate bytes that appear more frequently, making them poor candidates for
         * anchor-based
         *          scanning.
         */
         ...
         * @return The byte index in `[0, pattern.size())` with the lowest score, or `pattern.size()` when no position
         * is a
    ...
    namespace
    {
        ...
        // wrappers apply the offset exactly once on top of this result; scan_executable_regions also calls this
        // directly so its own final offset-application remains correct.
        DMK_NO_SANITIZE_ADDRESS
        ...
        // Shared guard for "pattern has no literal bytes". Returning start_address preserves backwards compatibility
        // for callers that rely on the degenerate "all wildcards matches anywhere" behaviour, but the call site is
        // almost always a bug. Logging once per public entry (rather than per internal find_pattern_raw iteration)
        // keeps the warning visible without flooding logs when the Nth-occurrence overload or scan_executable_regions
        // loops.
        ...
        // Shared precondition check for the public find_pattern overloads. Returns false when the caller must
        // short-circuit with nullptr (empty pattern or null start_address). Emits the all-wildcard warning itself so
        // callers do not duplicate it; in that case the caller still continues scanning.
    ...
    namespace
    {
                                          ...
                                          const Scanner::CompiledPattern &pattern) noexcept
        {
            ...
            //     "always match at region start", preserved for backward compatibility. The public wrappers log the
                                                ...
                                                size_t instruction_length)
    {
        ...
        // kernel-range instruction addresses (where intptr_t would be negative and signed overflow is UB). The
        // displacement is sign-extended first so negative disp32 values wrap to the correct 64-bit offset.
        ...
        // resolve to 0, a low guard-page address, or a kernel-range value; returning that as "success" would hand the
        // caller a pointer that faults on first use. plausible_userspace_ptr is pure arithmetic, so this guard adds no
    ...
    namespace
    {
        ...
        // detail::scan_module_* entry points. Walks the committed regions of [window_lo, window_hi) via VirtualQuery
        // and runs the per-region scan (scan_region_for_match, behind the fault guard) against every region whose base
        // protection is present in accept_mask, returning the Nth match (1-based, adjusted by pattern.offset) or
        // nullptr. The whole-process scanners pass [0, UINTPTR_MAX); the module-scoped scan passes the image's [base,
        // end) so only one contiguous image is searched.
        //
        ...
        //
        // Each region is scanned through the raw helper so the final `+ pattern.offset` applies exactly once (the
        // public find_pattern already applies offset; calling it here would double-apply). To find a signature that
        // straddles a protection split -- two adjacent accepted regions VirtualQuery reports separately because their
        // base protections differ (a sibling VirtualProtect carving part of .text into PAGE_EXECUTE_READWRITE is the
        // canonical case; VirtualQuery never coalesces regions with differing attributes) -- each accepted region's
        // scan is extended back by up to pattern_size - 1 bytes into the contiguous run of already-accepted regions it
        // abuts. The overlap is capped at pattern_size - 1 so a match lying wholly inside the previous region (already
        // counted there) can never be re-counted here, and bounded by the run start so it never reads past the bytes
        // the per-region gate proved readable.
        ...
        // for code-only scans: the whole-process scan_executable_regions and the prologue-recovery fallback, whose
        // rebuilt near-JMP can only ever overwrite a code prologue.
    ...
    // Nth match (1-based, adjusted by pattern.offset) or nullptr. They are the internal entry points the cascade
    // resolver (its own TU) calls instead of reaching the page-protection masks directly. Both reuse
    // scan_regions_filtered's per-region VirtualQuery protection gate, so a non-readable interior page (a
    // section-alignment gap, a guard page, a sibling VirtualProtect on part of the image) is skipped instead of
    // dereferenced. find_pattern_raw itself does an unguarded memchr / SIMD compare; on the no-fault path the gate is
    // what makes that safe, and scan_region_guarded backstops the gate against a concurrent decommit / reprotect that
    // would fault the read after the gate passed.
    ...
    // exactly as scan_region_guarded backstops the in-TU sweeps.
    ...
    const std::byte *DetourModKit::Scanner::scan_executable_regions(const CompiledPattern &pattern, size_t occurrence)
    {
        ...
        // Whole-process sweep: the window spans the entire user address space, so the clamp in scan_regions_filtered is
        // a no-op and the walk stops only when VirtualQuery runs off the end of the address space.
    ...
    const std::byte *DetourModKit::Scanner::scan_readable_regions(const CompiledPattern &pattern, size_t occurrence)
    {
        ...
        // PAGE_READWRITE / PAGE_WRITECOPY) and read-only heaps, plus the execute-readable variants. The semantic is
        // "find this pattern anywhere readable", so execute-readable code pages are intentionally included rather than
        // deduplicated against scan_executable_regions; callers wanting non-code matches post-filter. The window spans
    ...
    bool DetourModKit::Scanner::is_likely_function_prologue(std::uintptr_t addr) noexcept
    {
        ...
        // Reject bytes that never begin a real function prologue, so an AOB match that landed in inter-function padding
        // or past a function's end is filtered out instead of accepted as a target:
