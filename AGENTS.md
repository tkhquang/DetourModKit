...
- `external/safetyhook` -- inline/mid-function hooking (links Zydis/Zycore). v4 confines it to `src/hook_manager.cpp` + `include/DetourModKit/detail/hook_impl.hpp`; no public header includes or names it, so a consumer that includes `hook_manager.hpp` compiles with SafetyHook off its include path (`scripts/check_header_hygiene.py` enforces this)
...
#   DetourModKit_bench_scanner   -- Scanner::find_pattern, rare-byte anchor vs naive; a prefilter-isolation sweep
...
#                                   startup resolver batch scenario (serial cascades vs resolve_cascade_batch)
...
include/DetourModKit/    # Public headers -- one per module
  ...
  scan.hpp               # v4 scanning surface (namespace DetourModKit::scan): the value-semantic compiled AOB Pattern -- compile() (runtime, returns Result) and consteval literal() (compile-time, by value, so a typo in a hard-coded signature is a build error), backed by the heap/logger-free constexpr parse-and-anchor core in detail/pattern_core.hpp. The resolver surface collapses the six v3 resolve_cascade_* overloads into one scan::resolve(ScanRequest) plus a noexcept scan::resolve_batch (fork-join), where scope (Region), prologue fallback, per-request uniqueness, and candidate order (CandidateOrder) are request fields rather than function names; factory-only Candidate tiers (direct / rip_relative / rtti_vtable / string_xref) own their name/query strings and expose only const accessors, so a winning Hit name can never dangle and a mode/payload mismatch cannot be aggregate-initialized; ScanRequest is borrowed (built via borrow() or handed straight to resolve()) while OwnedScanRequest owns its ladder for stored/deferred resolution; scan(Pattern, Region) is the page-gated single-pattern scan and unchecked::find_pattern the raw caller-guarantees-readability twin; or_null / address_or flatten a Result<Hit> to an Address. Byte tiers prefilter on a runtime haystack-frequency anchor that overrides the Pattern's compile-time rarest-byte anchor when a sufficient image sample is available and falls back to it otherwise. The v4 resolver reuses the existing scan engine primitives (SIMD scan, RIP / string-xref / RTTI resolution, hooked-prologue recovery) and the legacy scanner.hpp surface below stays until the consumer migration
  scanner.hpp            # AOB pattern scanning (whole-process executable/readable regions + module-scoped and host-EXE cascade whose AddrCandidate rows also carry name/string resilience tiers above the byte tiers -- ResolveMode::RttiVtable resolves a mangled type name through Rtti::vtable_for_type and ResolveMode::StringXref resolves a literal through find_string_xref, both unique-only by construction and host-module-scoped when the cascade carries no explicit range; AOB syntax supports full-byte, full-wildcard, and per-nibble wildcard tokens; the region walk carries a pattern-length overlap across adjacent accepted regions so a signature straddling a protection split is still found; opt-in fork-join batch scanners resolve compiled patterns (scan_regions_batch whole process, scan_module_batch one image) and cascade requests (resolve_cascade_batch) concurrently, sharing read-only inputs across a transient worker pool; the prologue-recovery fallback recovers E9 near-jump, FF25 indirect-jump (both the separate-slot disp32 form and the 14-byte `FF 25 00000000 <abs64>` inline-target absolute form), and `mov rax, imm64; jmp rax` overwritten prologues) with a SIMD-tiered prefilter and verify (the self-provided dmk_memchr prefilter and the verify both tier SSE2/AVX2 at runtime, plus an opt-in AVX-512F+AVX-512BW verify tier behind the DMK_ENABLE_AVX512 build option); in-code constant extraction (read_code_constant, Zydis-backed); string-reference xref resolver (find_string_xref: locate an immutable literal, then its unique RIP-relative reference -- a fast lea/mov shape scan by default, or that scan plus an opt-in Zydis broad sweep for cmp/push/no-REX shapes; returns the load site, the enclosing function, or the cached global pointer slot)
  hook_manager.hpp       # Hook wrapper (inline, mid, VMT-object). SafetyHook is confined to detail/hook_impl.hpp; a mid-hook detour takes an opaque hook::MidContext read/written through gpr() / stack_pointer() / instruction_pointer() / xmm() accessors instead of naming the backend Context. Per-method VMT hooking (hook_vmt_method / with_vmt_method) is deferred to a later release.
  async_logger.hpp       # Lock-free MPMC queue logger (AsyncLogger); the queue/pool/message types live in detail/async_logger_internal.hpp
  async_logger_config.hpp # Lightweight OverflowPolicy + AsyncLoggerConfig (no queue/pool), so bootstrap.hpp stays light
  detail/                # Installed-but-internal headers (DetourModKit::detail): async_logger_internal.hpp, hook_impl.hpp -- the only header that names SafetyHook; pattern_core.hpp -- the constexpr heap/logger-free AOB parse-and-anchor core behind scan::Pattern
  ...
  anchors.hpp            # Declarative self-healing anchor registry over the vtable / cascade / code-constant / string-xref / manual backends, plus two-signal quorum corroboration (with sub-anchor independence checks), optional post-resolve validators and opt-in validator policies, opt-in parallel table resolution, a manifest quality diagnostic, and an address-independent evidence fingerprint that lets a future manifest diff tell "same evidence, new address" from "new evidence path" (Anchors::resolve, resolve_all, resolve_all_parallel, assess_quality, anchor_fingerprint)
  profile.hpp            # Per-game scan-tuning control plane: a value-semantic ScanProfile (broad string-xref default, cascade candidate order, backend deny-list) plus the profile-aware Anchors::resolve_with_profile / resolve_all_with_profile / resolve_all_with_profile_parallel (candidate order applies locally; deny-list fails closed)
  ...
  diagnostics_dump.hpp   # One-call aggregator (Diagnostics::collect -> Diagnostics::Snapshot) bundling the leak counters, get_hook_counts(), assess_quality(), and heal_report() summaries into one struct
src/                     # Implementation files. One .cpp per module, except a
                         ...
                         # scanner_parallel.cpp (fork-join batch scanner / cascade resolver) +
...
- **Anchor registry tests:** `tests/test_anchors.cpp` covers `anchors.hpp` (`resolve`, `resolve_all`, `resolve_all_parallel`, `anchor_status_to_string`), one case per resolvable kind (Manual literal, CodeOperand, RipGlobal, VtableIdentity fail-closed) plus the `CallArgHome` `Unsupported` path and the report-capacity behaviour. It also covers the optional post-resolve validator (accept, reject-fails-closed, context pass-through both ways, and the Manual / CallArgHome exemptions) and the `Quorum` kind (agreement, disagreement, one-signal-fails, null sub-anchor, rejected nesting, within-tolerance accept and reject, negative-tolerance fail-closed, the quorum's own validator applied to the corroborated value, and quorum propagation through `resolve_all`). The quorum-agreement cases pair genuinely independent sub-anchors (a `CodeOperand` plus a `Manual`), since the independence gate now rejects the dual-`Manual` shape; dedicated cases pin the `QuorumNotIndependent` rejections (pointer-equal, dual-Manual, same-backend-config) versus distinct-candidate-array acceptance, the opt-in `validate_manual` and `require_validator` policies (with the Quorum exemption), and `assess_quality`. A separate `AnchorFingerprintTest` suite covers `anchor_fingerprint`: determinism, label / candidate-name independence (cosmetic, not evidence), that the kind plus each kind's evidence inputs (vtable mangled name, cascade pattern bytes, code-operand decode params, string-xref shape flags, Manual literal) all change the hash, the order-independent Quorum combine (with match-mode / tolerance and null sub-anchor handling), and the load-bearing same-evidence-different-address invariant (the same AOB pattern resolved in two allocations yields two addresses but one fingerprint). The per-game scan profile (`profile.hpp`) has its own suite, `tests/test_profile.cpp`: `apply_profile` broad-widening, `order_candidates` permutations, `candidate_order_to_string` totality, the fail-closed (non-substituting) deny-list at the anchor and quorum levels, and serial-vs-parallel report equivalence for `resolve_all_with_profile_parallel`.
...
|--------|--------------|-------------------|
| Scanner | Stateless -- inherently safe; the opt-in batch scanner (`scan_regions_batch` / `scan_module_batch`) shares immutable `CompiledPattern`s read-only across a transient fork-join worker pool (no per-pattern mutation; `compile_anchor()` must precede the batch), and `resolve_cascade_batch` shares caller-owned cascade tables read-only while dispatching each request through the existing serial resolver. Both write each result slot from one worker via an atomic cursor and join before returning | N/A (startup only; the batch scanner and batch cascade resolver are setup/control-plane, never callback-safe) |
...
- **Do not expose** implementation-only container or entry types as top-level public API in public headers -- place them in `namespace detail` (e.g. `detail::VmtHookEntry`, and the async-logger `detail::StringPool` / `detail::LogMessage` / `detail::DynamicMPMCQueue` in `detail/async_logger_internal.hpp`) or an internal header so they stay out of the documented API surface. A third-party backend type that a public signature would otherwise name is pimpl'd behind a forward-declared `Impl` whose definition lives in a `detail/` header included only by the owning `.cpp` (the SafetyHook objects behind `detail/hook_impl.hpp`); a backend value the API must hand to a callback is surfaced as an opaque pass-through type plus free accessors (`hook::MidContext` with `gpr()` / `stack_pointer()` / `instruction_pointer()` / `xmm()`), never by leaking the backend type into the header.
