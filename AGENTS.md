...
include/DetourModKit/    # Public headers -- one per module
  defines.hpp            # v4 portability primitives in one place: the x86-64 arch gate, DMK_FORCE_INLINE, the DMK_FLAG_ENUM flag-enum operator generator, DMK_LIFETIMEBOUND, the DMK_API visibility marker, and the short `dmk` = `DetourModKit` namespace alias
