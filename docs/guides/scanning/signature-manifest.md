...
The reserved keys (`kind`, `binding`, `pattern`, ...) and the `manifest` / `sig.` section prefixes must be canonical lowercase. The backend is case-sensitive, so a miscased key or prefix fails the parse (`MalformedLine`) before collision detection sees it. Everything else is payload taken verbatim: the `<label>`, the `mangled` type name (`.?AVCAIController@@`), and the `pattern` bytes (`F3 0F 11`) keep their original casing.

- The `.rung.<N>` marker is case-sensitive, but a miscased marker is not a structural error. `[sig.foo.RUNG.0]` reads as an ordinary record whose label is `foo.RUNG.0`, not as a rung of `sig.foo`.
- A key that repeats exactly or differs only by surrounding whitespace is a colliding identity (`ManifestIdentityCollision`), rejected before it can merge.
- Section identities also collide after ASCII case folding, so `[sig.Foo]` beside `[sig.foo]` is the same rejection.
- Enum-token values stay case-insensitive, so `kind = RipGlobal` reads the same as `kind = rip_global`. `serialize_checked` writes them back in lowercase.
