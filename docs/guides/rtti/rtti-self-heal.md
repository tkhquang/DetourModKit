...
- **Per-group latch.** Each `add_group` is independent. A group that resolves stops being scanned; a sibling that has not keeps retrying. `all_resolved()` reports whether every group has latched, counting a group registered from inside a running `tick()` from the moment `add_group` returns.
