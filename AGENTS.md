...
#                                   (dmk_memchr vs libc memchr on a 64 MiB unique-anchor buffer) and a deep-verify
#                                   throughput scenario (the AVX-512 verify tier's throughput gate harness)
...
include/DetourModKit/    # Public headers -- one per module
  ...
  anchors.hpp            # Declarative self-healing anchor registry over the vtable / cascade / code-constant / string-xref / manual backends, plus two-signal quorum corroboration (with sub-anchor independence checks), optional post-resolve validators and opt-in validator policies, and a manifest quality diagnostic (Anchors::resolve, resolve_all, assess_quality)
  profile.hpp            # Per-game scan-tuning control plane: a value-semantic ScanProfile (broad string-xref default, cascade candidate order, backend deny-list) plus the profile-aware Anchors::resolve_with_profile / resolve_all_with_profile (candidate order applies locally; deny-list fails closed)
