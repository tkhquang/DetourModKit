...
That patch transaction is not atomic with its own outcome, which is why DMK does not trust the backend's return value on its own. The bytes are written inside the trap window, and the transaction can still fail afterwards while restoring page protection -- so an error can be returned over a fully committed patch, and over a fully committed restore. `Hook::enable`, `Hook::disable`, and `~Hook` therefore read the patch window back and classify it against both the saved prologue and the exact encoding the backend emitted. The classification, not the return value, decides what gets published: a hook whose patch committed under a failed transaction reports `is_enabled() == true` and returns `ErrorCode::BackendFailed` rather than a false `Disabled` over a live detour; a restore in the same position lets `Hook::disable` publish `Disabled` and close the call gate, while the trampoline stays mapped until `~Hook` frees it, which it does only once its own witness has confirmed the target no longer jumps into it. Bytes matching neither comparand belong to some other writer, and both toggles refuse rather than overwrite them, because the backend writes unconditionally; teardown pins the backend for the same reason.
