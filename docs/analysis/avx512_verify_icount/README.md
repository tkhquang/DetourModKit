...
This directory captures a no-hardware bench signal for the opt-in AVX-512 scanner verify tier (`DMK_ENABLE_AVX512`). It records *instruction counts*, not wall-clock time, because AVX-512 wall-clock can only be measured on a real AVX-512 host (the dispatch-only `throughput-gate` job); Intel SDE, which is what makes this measurable without that hardware, emulates the ISA and its timing is not representative of real silicon. What SDE *can* measure deterministically on any host is how many guest instructions each verify tier executes for the same work.
...
- Source: the `verify-icount-proxy` job of the SIMD-tier CI workflow
...
3. **Correctness is a separate, stronger signal.** The `-spr` correctness run (a sibling CI job) executes the full scanner suite under emulated AVX-512 and requires results identical to the scalar/AVX2 path. That validates the 64-byte body; this directory only addresses whether the tier is worth turning on.
...
- **Instruction count is not wall-clock.** This whole directory is a proxy. The real `>= 1.30x` throughput gate runs only on a self-hosted AVX-512 machine (the `throughput-gate` job); until it clears, the tier stays opt-in (`DMK_ENABLE_AVX512` OFF).
