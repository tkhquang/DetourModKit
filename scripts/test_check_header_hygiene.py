...
def test_scanner_gate_accepts_a_conditional_nested_inside_the_asan_chain():
    source = ("#if defined(__SANITIZE_ADDRESS__)\n"
              "#if defined(_MSC_VER)\n"
              "    std::memcpy(out, in, n);\n"
              "#endif\n"
              "#endif\n")
    _expect(unguarded_libc_memory_calls(source) == [], "a call nested inside an armed chain was rejected")
