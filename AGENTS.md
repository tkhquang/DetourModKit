...
include/DetourModKit/    # Public headers -- one per module
  ...
  async_logger_config.hpp # Lightweight OverflowPolicy + AsyncLoggerConfig (no queue/pool), so logger.hpp / dmk.hpp stay light
  detail/                # Installed compile-visible support (DetourModKit::detail): pattern_core.hpp (the constexpr heap/logger-free AOB parse-and-anchor core behind scan::Pattern) plus the demoted compile-visible / umbrella-reachable headers: event_dispatcher.hpp, win_file_stream.hpp, worker.hpp, drift_manifest.hpp (each installed because a public header or the dmk.hpp umbrella still includes it). True-private impl (the SafetyHook bridge, the async-logger queue, srw_shared_mutex) lives in the non-installed src/internal/.
  logger.hpp             # Process logger value facade reached through log(); construct your own Logger for a dedicated sink
  detail/win_file_stream.hpp # Win32 shared-access file stream (CreateFile backend); demoted to detail/ -- include as DetourModKit/detail/win_file_stream.hpp
  ...
  dmk.hpp                # v4 umbrella + process lifecycle (namespace DetourModKit): the RAII Session (start(ModInfo) -> Result<Session> synchronous path, or bootstrap(ModInfo, on_ready) DllMain worker path) whose destructor runs the ordered subsystem teardown (scope clear -> config watcher -> input -> memory cache -> config -> logger last), each leaf embedding its own loader-lock guard; bootstrap_detach(lpReserved) (NULL -> ordered ~Session, non-NULL -> abandon), request_shutdown, module_handle, and the on_logic_dll_unload / on_logic_dll_unload_all hot-reload helpers. Also aggregates every public module header and the DMK / DMKConfig / ... convenience aliases
