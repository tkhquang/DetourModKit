...
| `ExportName` | `scan::resolve_export` | A named export, resolved by a walk of its module's PE Export Address Table (the most update-resilient kind) |
...
`Anchor::kind` defaults to `Unset`, so a designated-initializer table entry that forgets to set `kind` fails closed (`AnchorStatus::Failed`) rather than a silent resolve as a trusted `Manual` address 0. A declared `Unset` is always a mistake. It exists so the mistake fails safe. Always set `kind` explicitly.

A `StringXref` anchor is the most update-resilient kind that anchors on in-image content (an `ExportName` is more resilient still when the target is a named export, see below). It locates an immutable string literal in the image's read-only data and resolves the unique RIP-relative `lea` / `mov` that references it. It returns that instruction, or one of two alternatives:
...
`CallArgHome` is a reserved enumerator for a future prologue-dataflow backend (a map of a call argument to its current register or stack home). A declaration of it now keeps a registry table forward-compatible.

### Corroborating a critical target (`Quorum`)

For a target whose breakage is costly, a single signal can be too weak. One AOB or code constant can resolve to a coincidental match after a patch. A `Quorum` anchor votes across `M` independent sub-anchors and accepts only when at least `N` of them resolve *and agree* (N-of-M voting). A coincidental match therefore has to fool a whole cluster at once. Unlike a strict pair, the target still corroborates when a game patch breaks some of the signals, as long as `N` of the rest still agree. List the members in `quorum_members` (non-owning pointers into your own anchor storage, kept alive across the `resolve` call), set `quorum_threshold` to `N`, and pick the agreement policy:
...
`quorum_members` are non-owning pointers into your own anchor storage, so keep the members alive across the `resolve` call. `quorum_threshold` is `N`. `0` (the default) means unanimous, so a two-member quorum with the default is the strict 2-of-2 corroboration. A quorum is corroboration, so an explicit `N` below 2 or above the member count is malformed. `QuorumMatch::ExactValue` (the default) requires identical values. `QuorumMatch::WithinTolerance` accepts a gap no larger than `quorum_tolerance` (a negative tolerance never accepts). A single qualifying cluster commits its smallest qualifying center, independent of declaration order. If two qualifying centers disagree, resolution returns `AnchorStatus::QuorumAmbiguous` instead of a choice by order. Fewer than `N` agreeing members and malformed declarations return `AnchorStatus::Failed`.

Independence is checked twice, because corroboration only means something when every vote is separate evidence. Before resolution, five patterns make the quorum `QuorumNotIndependent`:

- Reuse of one `Anchor`.
- A pair of two `Manual` literals.
- A shared candidate rung.
- Selection of two operands from one declared instruction.
- The same effective export spelled as both scoped and explicit.

This comparison uses evidence content rather than view storage, ignores cosmetic names and operand selectors, and compares ladder atoms as sets. After resolution, the winning physical sources are compared too. Overlapping byte-pattern spans, or a decoded instruction site that overlaps another winner's span, are one failure domain even when different patterns, bounded jumps, result markers, or walk-backs selected them. Exports are correlated by the entry they resolved through rather than by the name that asked for it. Two names that share an `AddressOfFunctions` slot, or that carry the same function RVA in one image, count once. That is what a declaration alone cannot decide, because which names alias is a property of the live export table. Equal resolved values alone are not dependency, so distinct instructions that reference the same target remain independent, and an export still corroborates against a signal read from somewhere else.

### Post-resolve validators

Any backend-resolved anchor (not `Manual` or `CallArgHome`) can carry an optional `validator`: a `bool(*)(std::int64_t value, const void* context) noexcept` predicate run on the resolved value immediately before it is accepted. A return of `false` fails the anchor closed (`Failed`, value reset to 0), identical to a backend miss, so the caller re-heals by a re-run of `resolve`. Use it to assert a domain invariant a generic backend cannot know: the target lies in an expected sub-range, a displacement points into `.rdata`, or the site begins with a plausible prologue (`scan::is_likely_function_prologue`). `validator_context` is an opaque pointer forwarded verbatim to the predicate (nullptr if unused). For a `Quorum`, the validator runs once on the corroborated value after the members vote.

Two opt-in policy flags harden this further (both default to preservation of the existing behaviour):

- `validate_manual = true` runs the `validator` on a `Manual` anchor too, instead of a take of the pinned literal unchecked, so you can assert a domain invariant on a hand-pinned constant.
- `require_validator = true` rejects a backend-resolvable anchor that carries no `validator` at all (status `Failed`). A function or global target with no domain check is treated as unverified. A `Quorum` is exempt, because its N-of-M corroboration already is the verification.

When a target can be expressed more than one way, prefer the most update-resilient backend: `ExportName` > `StringXref` > `VtableIdentity` > `RipGlobal` > `CodeOperand`. A `Quorum` votes over several of those to raise confidence further. `Manual` is the last resort.
...
A `RipGlobal` anchor scans `Pages::Readable` by default, which reaches a `Direct` rung that resolves a plain global address in `.rdata` / `.data`. When every rung of a `RipGlobal` ladder anchors on an in-image instruction (a RIP-relative reference whose pattern is code), set `.pages = sc::Pages::Executable` on the anchor. A coincidental byte twin in a data page then cannot alias the site and demote a unique resolve to a fail-closed ambiguity. A manifest persists this non-default policy as `pages = executable`. An absent key remains `Readable` for compatibility. `CodeOperand` needs no such knob. `read_code_constant` scans code bytes and verifies that its final resolved site is execute-readable before it decodes.

For `CodeOperand`, `byte_width = 0` preserves the decoded value and 1 through 8 narrows a non-RIP constant to its low-order bytes before sign-extension. A value above 8 is malformed and fails before the scan. RIP-relative memory operands always resolve to their absolute target, so the width does not narrow that address.

A `Pages::Readable` scan reads every committed readable page of its scope, which includes the allocator pages a caller's own copies of the query bytes live on. DMK always excludes the query representations it owns (the compiled pattern's byte and mask buffers, the `scan::Pattern` object's own storage, the candidate ladder and its owned literals). It cannot discover a copy the caller retained. A readable scope confined to one mapped image or one allocation is therefore fine. A wider one (`Region::whole_process()`, or any span across many allocations) returns `ErrorCode::NotAuthoritative` rather than an address that can be the query finding itself. The scan API can make a wider executable-page sweep authoritative, because query bytes are data. The anchor trust layer is stricter. `VtableIdentity`, `RipGlobal`, `CodeOperand`, and `StringXref` require their common scope to stay inside one reserved allocation regardless of page class. They re-check that boundary after validators and witness construction, because uniqueness depends on the whole searched range. Use a narrow module or allocation `Region`. Cross-module quorum evidence remains supported through explicit `ExportName::export_module` members. A refusal reaches an anchor caller as `AnchorStatus::Failed`, because `ResolvedAnchor` carries no error code. Callers that use the scan API directly can instead use an executable sweep or declare retained query copies through `ScanRequest::exclusions` or the exclusion-taking `scan::scan` overload.
...
Telemetry alone only describes health. `anchor::evaluate_gate` turns it into a decision. Startup can then safe-disable a feature when its anchors did not resolve well enough, instead of a patch of the game on addresses the manifest did not verify:
...
`anchor::anchor_fingerprint(anchor)` hashes only an anchor's *resolution evidence*: its `AnchorKind` plus the inputs that backend uses.

- `VtableIdentity`: the mangled name.
- `RipGlobal` / `CodeOperand`: the compiled-Pattern bytes plus wildcard mask and decode parameters, and the non-default `RipGlobal` page class.
- `StringXref`: the literal and shape flags.
...
- `image` - the `scan::ImageIdentity` of the module that owns the resolved address. Absent for a `Scalar` (a code operand or `Manual` literal, which is a constant rather than a location), an unresolved entry, or a non-module synthetic address. Resolution captures each module-backed evidence owner before its backend walk and captures the address-domain value owner before validation. Every key and its mapping class are re-checked after commit and witness construction. A scope-backed member also propagates its single-allocation obligation through a `Quorum`. The witness copies the accepted value-owner key rather than a resample of it. Four conditions return `AnchorStatus::Failed` with value 0 and no witness: an unreadable module identity, an image or synthetic class transition, an allocation split, or a flagged same-base replacement. The last is a replacement whose PE layout identity differs from the captured one, found by a validator or between two members of a `Quorum`. Two limits are worth a plain statement. The replacement check is layout-identity equality, so a replacement that reproduces the captured timestamp, `SizeOfImage`, and section-table digest is indistinguishable from the original here. Use `evidence` below for content, which is what an in-place patch moves. And the comparison is between snapshots taken at the recorded points (before the backend walk, before validation, after commit, after witness construction), not a continuous watch. It proves the identity held across those points, not that nothing happened between them.
...
- `evidence` - the literal bytes of the span the winning byte-pattern rung matched, captured during that match. Present only when a `RipGlobal` anchor wins on a `Direct` or RIP-relative rung, the one path that returns the span it matched. Evidence is absent when a rung wins through a structure instead (RTTI vtable, string xref), and absent for the `VtableIdentity`, `StringXref`, `ExportName`, `Manual`, and `Quorum` kinds. It is also absent for `CodeOperand`, which locates through a byte pattern but returns a decoded `Scalar` rather than the site. `scan::WinningEvidence` in `scan.hpp` owns the content-versus-layout contract and the same-traversal capture guarantee, and the [AOB signatures guide](../../misc/aob-signatures.md) covers the immutable sweep snapshot behind a RIP-relative winner.
...
Resolution is idempotent and side-effect-free, so the registry keeps no hidden state. The resolved value lives in the `ResolvedAnchor` you hold. When a later sanity check shows a cached value has gone stale, re-run `resolve` on that one anchor:
...
Like the underlying resolvers, the registry defaults to the host EXE and accepts an explicit `Region`. Scope is load-bearing for correctness, not only ergonomics. The same vtable name or instruction shape can exist in several loaded modules. Pass the game module's range when the target lives in a separate DLL.
