...
namespace
{
    ...
    TEST(VersionTest, AtLeastComparisonsAreCorrect)
    {
        ...
        // dropped the version back below 4.1.0 is caught here even if the macros themselves were edited in lockstep.
        // The 4.1.0 floor is satisfied. The next patch and minor on this major are not yet reached.
        EXPECT_TRUE(DMK_VERSION_AT_LEAST(4, 0, 0));
        EXPECT_TRUE(DMK_VERSION_AT_LEAST(4, 1, 0));
        EXPECT_FALSE(DMK_VERSION_AT_LEAST(4, 1, 1));
