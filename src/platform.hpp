...
namespace DetourModKit::detail
{
     ...
     *       with the module pinned); a spurious false would join a thread under the loader lock and deadlock the host
     *       on unload, so uncertainty must bias toward true.
    ...
    /**
     * @brief Pins the current module to prevent unload while detached threads are still running.
     * @details Uses GET_MODULE_HANDLE_EX_FLAG_PIN to permanently increment the module reference count. Safe to call
     *          from DllMain. This ensures that code pages and static data remain valid for detached threads during the
     *          FreeLibrary path.
    ...
    inline bool pin_current_module() noexcept
    {
        ...
        if (!GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_PIN,
                                reinterpret_cast<LPCWSTR>(&pin_current_module), &pin))
        {
