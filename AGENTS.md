...
#   DetourModKit_bench_scanner   -- find_pattern, rare-byte anchor vs naive; a prefilter-isolation sweep
#                                   (dmk_memchr vs libc memchr on a 64 MiB unique-anchor buffer), a deep-verify
#                                   throughput scenario (the AVX-512 verify tier's throughput gate harness), and a
#                                   startup resolver batch scenario (serial cascades vs scan::resolve_batch)
#   DetourModKit_bench_memory    -- validation predicate vs direct SEH-guarded read / chain primitives (hot-path cost);
#                                   phase [9] is a warm-hit is_readable throughput-under-contention scan that exercises
#                                   the striped reader counters
...
include/DetourModKit/    # Public headers -- one per module
  defines.hpp            # v4 portability primitives in one place: the x86-64 arch gate, DMK_FORCE_INLINE, the DMK_FLAG_ENUM flag-enum operator generator, DMK_LIFETIMEBOUND, the DMK_API visibility marker, and the short `dmk` / `DMK` = `DetourModKit` namespace aliases
  address.hpp            # v4 Address value type: a strong machine-address type with constexpr offset/align_up/rip arithmetic and the single audited reinterpret_cast surface (the templated pointer ctor, as<T>(), ptr<T>())
  region.hpp             # v4 Region value type (a [base, base+size) half-open span) with named scope factories (host() / module_named() / whole_process()) plus the backend-neutral Prot read/write/execute flag enum
  error.hpp              # v4 unified error idiom: the eight per-domain enums folded into one high-byte-tagged ErrorCode superset (category() recovers the subsystem), a trivially-copyable Error, Result<T> = std::expected<T, Error>, and the DMK_TRY / DMK_TRY_VOID propagation macros
  scan.hpp               # v4 scanning surface (namespace DetourModKit::scan): the value-semantic compiled AOB Pattern -- compile() (runtime, returns Result) and consteval literal() (compile-time, by value, so a typo in a hard-coded signature is a build error), backed by the heap/logger-free constexpr parse-and-anchor core in detail/pattern_core.hpp. The resolver surface is one scan::resolve(ScanRequest) plus a noexcept scan::resolve_batch (fork-join) that returns Result<vector<Result<Hit>>> so the outer Result carries a whole-batch OOM failure a caller cannot silently index past (mirroring hook::install_all), where scope (Region), prologue fallback, per-request uniqueness, candidate order (CandidateOrder), and byte-tier page class (Pages, so a resolve can restrict its byte tiers to executable pages) are request fields rather than function names; factory-only Candidate tiers (direct / rip_relative / rtti_vtable / string_xref) own their name/query strings and expose only const accessors, so a winning Hit name can never dangle and a mode/payload mismatch cannot be aggregate-initialized; ScanRequest is borrowed (built via borrow() or handed straight to resolve()) while OwnedScanRequest owns its ladder for stored/deferred resolution; scan(Pattern, Region, occurrence, Pages) is the page-gated single-pattern scan (Nth-occurrence plus Readable/Executable page selection) and unchecked::find_pattern the raw caller-guarantees-readability twin; or_null / address_or flatten a Result<Hit> to an Address. Byte tiers prefilter on a runtime haystack-frequency anchor that overrides the Pattern's compile-time rarest-byte anchor when a sufficient image sample is available and falls back to it otherwise. The reusable scan engine (SIMD scan, page-gating, RIP / string-xref / RTTI resolution, hooked-prologue recovery) lives in the NON-installed private headers under src/internal/ (scan_engine / scan_pages / scan_prologue_recovery / scan_batch, namespace DetourModKit::detail), so no engine header ships; scan.hpp also exposes the standalone capability surface so the scan module keeps full scanner capability: resolve_rip_relative / find_and_resolve_rip_relative (+ the PREFIX_* opcode constants), find_string_xref (+ StringRefQuery / StringEncoding / XrefReturn), read_code_constant (+ CodeConstant / OperandKind), is_likely_function_prologue, and active_simd_level (SimdLevel). The declarative anchor registry is now a first-class module (anchor.hpp, namespace anchor) that composes those four backends
  anchor.hpp             # Declarative anchor registry (namespace DetourModKit::anchor): AnchorKind (VtableIdentity / RipGlobal / CodeOperand / StringXref / Manual / CallArgHome / Quorum), QuorumMatch (ExactValue / WithinTolerance), AnchorStatus (Unresolved / Resolved / Failed / Unsupported / QuorumNotIndependent); flat designated-init Anchor aggregate (site = span<const scan::Candidate>, optional AnchorValidator; the module range is NOT a field -- scope is a Region parameter on the resolve* functions, defaulting to Region::host()); ResolvedAnchor {label, kind, status, value}; AnchorQuality; ScanProfile (default_broad_string_xref, candidate_order = scan::CandidateOrder, deny_backend, is_denied); free functions resolve / resolve_all / resolve_all_parallel / assess_quality / anchor_fingerprint / apply_profile / resolve_with_profile / resolve_all_with_profile / resolve_all_with_profile_parallel (the resolve* entry points take the Region scope). anchor_fingerprint hashes the COMPILED Pattern content (bytes + wildcard mask + decode params), not AOB source text. Quorum corroboration performs sub-anchor independence checks. Candidate ordering reuses scan::CandidateOrder / scan::order_candidates
  hook.hpp               # v4 hooking surface (namespace DetourModKit::hook): free verbs inline_at / mid_at returning a move-only RAII Hook handle whose destructor unhooks (the loader-lock leaf discipline applies: under the loader lock it pins + leaks instead of restoring). A by-value guarded Hook::call<Ret>(Args...) twin sits beside the named-unguarded typed trampoline Hook::original<Fn>(); the target is an Address or a deferred scan::OwnedScanRequest. The declarative install_all(span<HookSpec>) -> vector<InstallOutcome> table folds the install ceremony into rows with a per-row Mandatory/BestEffort Severity (a Mandatory miss fails-and-rolls-back, a BestEffort miss is recorded and skipped). Options carries the prologue policy (Fail by default / Relocate) and fail_if_already_hooked; is_target_hooked(Address) is the exact same-kit query. A mid-hook detour (MidHookFn) takes an opaque hook::MidContext read/written through gpr() / stack_pointer() / resume_stack_pointer() / instruction_pointer() / flags() accessors plus a read-only xmm() view, never naming the backend Context. VMT object hooking is the RAII VmtHook (vmt_for clones, apply_to / remove_from, every vptr restored on drop); per-method typed hooking rides the same handle (hook_method<Fn>(index, detour) redirects one cloned slot on every applied object, original<Fn>(index) returns the typed pre-hook function pointer, remove_method(index) lifts one method), with an SRWLOCK-backed shared-read snapshot in original() against the exclusive-write hook_method / remove_method / apply_to / remove_from. SafetyHook is confined to the sanctioned internal islands (`src/hook.cpp` + non-installed `src/internal/hook_backend.hpp`, and `src/internal/input_intercept.cpp` for active-input XInput hooks); a process-wide ledger (src/internal/hook_ledger.hpp -- not a public registry) backs exact duplicate detection and layered-same-target teardown-order tracking. Fallible operations return Result<T> over the unified ErrorCode (the former HookError folded in).
  async_logger.hpp       # Lock-free MPMC queue logger (AsyncLogger); the queue/pool/message types live behind its pimpl in the non-installed src/internal/async_logger_queue.hpp
  async_logger_config.hpp # Lightweight OverflowPolicy + AsyncLoggerConfig (no queue/pool), so logger.hpp / DetourModKit.hpp stay light
  detail/                # Installed compile-visible support (DetourModKit::detail): pattern_core.hpp (the constexpr heap/logger-free AOB parse-and-anchor core behind scan::Pattern) plus the demoted compile-visible / umbrella-reachable headers: event_dispatcher.hpp, worker.hpp, drift_manifest.hpp (each installed because a public header or the DetourModKit.hpp umbrella still includes it). True-private impl (the SafetyHook bridge, the async-logger queue, srw_shared_mutex, the Win32 file stream) lives in the non-installed src/internal/.
  logger.hpp             # Process logger value facade reached through log(); construct your own Logger for a dedicated sink
  src/internal/win_file_stream.hpp # Win32 shared-access file stream (CreateFile backend, namespace DetourModKit::detail); NON-installed private impl held behind logger.hpp / async_logger.hpp as a shared_ptr<detail::WinFileStream> forward declaration -- moved out of detail/ in v4
  config.hpp             # v4 configuration surface (namespace DetourModKit::config): free functions over the process config registry -- bind<T> (the atomic hot path) / bind_int / bind_float / bind_bool / bind_string / bind_combos / bind_parsed (atomic uint32 through a parse fn) / bind_log_level, load / reload / log_all / clear, plus the SectionBinder (config::section("X") drops the repeated section arg) and the Ini handle that exposes the same operations + section(). Also the input-binding fusions press_combo / hold_combo that pair an INI combo item with an input press/hold binding, rebind it on reload, carry an optional per-binding "<ini_key>.Consume" suppression facet (consume_flag), and return an input::BindingGuard -- the hold guard synthesizes one balancing on_state_change(false) on cancel so a torn-down hold cannot strand the consumer as held (the teardown gate is the internal src/internal/input_hold_gate.hpp HoldGate). reload_hotkey installs a press_combo that requests a reload off a background servicer. Config is fail-soft: bind / load return void, reload returns bool, enable_auto_reload returns AutoReloadStatus; there is no Result and no per-domain error enum. The filesystem watcher is folded in -- there is no separate watcher header, only enable_auto_reload / disable_auto_reload over the non-installed src/internal/config_watcher.hpp DetourModKit::detail::ConfigWatcher (ReadDirectoryChangesW). config depends on input, never the reverse
  input.hpp              # v4 input surface (namespace DetourModKit::input): the Input process singleton (Input::instance(), unifying the binding-management facade and the poll engine) owns the poll thread, the binding set, and the process-global interception layer. Register a ComboBinding (designated-init aggregate: name / trigger / combos / consume / on_press / on_state_change) via input::register_combo / Input::register_combo -> a move-only BindingGuard; a Scope owns guards and releases them in reverse insertion order (input::scope() is the process-default). Trigger{Press,Hold} (to_string(Trigger)); KeyCombo / KeyComboList live here. Input::start(Settings) / shutdown / is_running / is_active(name|token) / acquire_token / rebind / set_consume / set_require_focus / remove_bindings_by_name / clear_bindings. Generation-checked BindingToken handles let a high-frequency consumer query a binding without the per-call name hash, failing closed on any reshape. The poll/edge engine (detail::InputPoller / detail::InputBinding) and the interception layer live in the non-installed src/internal/ (input_poller.hpp / input_intercept.hpp / input_key_cache.hpp / input_hold_gate.hpp); the installed header names no Win32 or backend type
  input_codes.hpp        # Unified InputCode type and named key tables (incl. WheelUp/Down/Left/Right)
  src/internal/srw_shared_mutex.hpp # Opaque Windows SRWLOCK SharedLockable wrapper (NON-installed private impl; no public header includes it -- moved from include/ to src/internal/ in v4)
  memory.hpp             # v4 memory surface (namespace DetourModKit::memory): read<T>/read_into/write<T>/write_bytes/write_in_place (Result<T>) over Address/Region, sharded region cache, walk (fault-guarded pointer chain), is_plausible_ptr, ProtectGuard, module_of/is_module_loaded
  rtti.hpp               # MSVC RTTI walker (namespace DetourModKit::rtti, over Address / Result<T>): type_name_of, vtable_is_type, find_in_pointer_table (std::optional<Address>, caller-owned std::atomic<Address> cache) + reverse name-to-vtable (vtable_for_type -> std::optional<Address>; vtables_for_type -> a std::size_t count written into a caller Address[] buffer; self-contained TypeIdentity that owns its mangled name)
  rtti_dissect.hpp       # Reverse RTTI dissection + self-healing offsets (identify_pointee_type, identify_pointee_typed / heal_landmark / solve_fingerprint returning Result<...>, reverse_scan_block; self-contained Landmark owns its expected_mangled) + drift-telemetry report (heal_report, DriftEntry) + the frame-driven rtti::HealScheduler (start(HealConfig) -> Result<HealScheduler>, fixed-interval retry-until-resolved groups via add_group / tick, each group healing through a HealRun into a caller-owned std::atomic<std::ptrdiff_t> slot)
  detail/drift_manifest.hpp # Durable serialize/parse of the self-heal drift report (DriftEntry) so drift can be saved and diffed across game versions; demoted to detail/. parse_drift_report / read_drift_report_from_file now return Result<std::vector<DriftRecord>>; check errors via .error().code (ErrorCode::MissingHeader / MalformedLine / FileOpenFailed -- the former ManifestError enum folded into ErrorCode::Manifest category)
  detail/event_dispatcher.hpp # Typed pub/sub with RAII subscriptions (header-only); demoted to detail/ -- include as DetourModKit/detail/event_dispatcher.hpp
  ...
  ../DetourModKit.hpp    # v4 umbrella + process lifecycle (namespace DetourModKit), at the include root beside the DetourModKit/ module dir so consumers write <DetourModKit.hpp>: the RAII Session (start(ModInfo) -> Result<Session> synchronous path, or bootstrap(ModInfo, on_ready) DllMain worker path) whose destructor runs the ordered subsystem teardown (scope clear -> config watcher -> input -> memory cache -> config -> logger last), each leaf embedding its own loader-lock guard; bootstrap_detach(lpReserved) (NULL -> ordered ~Session, non-NULL -> abandon), request_shutdown, module_handle, and the on_logic_dll_unload / on_logic_dll_unload_all hot-reload helpers. Aggregates every public module header (the short DetourModKit aliases dmk and DMK live in defines.hpp)
  detail/worker.hpp      # StoppableWorker RAII std::jthread wrapper; demoted to detail/ (TU moved to src/internal/worker.cpp) -- include as DetourModKit/detail/worker.hpp
  diagnostics.hpp        # Consumer-queryable counters for intentional loader-lock leak/detach events per subsystem, plus a process-wide diagnostic event bus that surfaces scanner-fault (region-skip) and hook install/enable/disable/remove transitions as typed EventDispatcher events (diagnostics::scanner_faults, hook_lifecycle). Also contains the folded-in one-call aggregator: diagnostics::collect(drift_report, anchor_report) -> diagnostics::Snapshot bundling the leak counters, hook counts (hooks_total / hooks_active / hooks_disabled, derived from the hook-lifecycle transition stream), anchor quality (anchor::AnchorQuality via anchor::assess_quality), and the heal-drift summary into one struct. diagnostics_dump.hpp was deleted; its surface now lives here
src/                     # Implementation files. One .cpp per module, except a
                         # cohesive module may split into sibling TUs sharing one
                         # public header: the scan module splits into public
                         # per-concern TUs (scan_matching / scan_resolution /
                         # scan_candidates / scan_rip_relative / scan_string_xref /
                         # scan_code_constant) over a NON-installed private engine
                         # under src/internal/ (scan_engine / scan_pages /
                         # scan_prologue_recovery / scan_batch / scan_shared,
                         # namespace DetourModKit::detail), with Zydis confined to
                         # scan_string_xref / scan_code_constant;
                         # rtti.cpp + rtti_dissect.cpp via rtti_internal.hpp
  internal/              # NON-installed private scan engine headers + TUs; never shipped
...
- **Pointer-chain tests:** `tests/test_memory_chain.cpp` is a deliberate second suite for the public `memory.hpp` surface, kept separate from `test_memory.cpp` because the single-fault-frame pointer-chain primitives (`is_plausible_ptr`, `walk`, and the `read<T>` / `read_into` at the resolved `Address`) walk in-process pointer chains and need no cache or game-memory state, whereas `test_memory.cpp` drives the sharded cache, read/write, and module-range paths. Both suites bind to the same header; this split is intentional for state isolation (one fault frame vs cache/game-memory state). A second same-module split exists for a different reason: `tests/test_string.cpp` (the `string::trim` cases) and `tests/test_format.cpp` (the `format::*` cases) both bind to the header-only `format.hpp`, split by API surface rather than state.
...
- **Reverse-RTTI by-name tests:** `tests/test_rtti_reverse.cpp` covers `rtti.hpp`'s reverse resolvers (`vtable_for_type`, `vtables_for_type`, `TypeIdentity`). It builds synthetic COL/TypeDescriptor/vtable fixtures in the test exe's data segment (so the prelude's module bound-check accepts them) and resolves them through a tight pool range for speed, with one case driving the real PE-section walk via `Region::host()`. It exercises single- and multiple-inheritance (one name -> many sub-object vtables), the `COL.offset == 0` primary selection, ambiguous-name fail-closed, and the warm-cache identity path.
- **Code-constant tests:** `tests/test_code_constant.cpp` covers `scan::read_code_constant`. It plants known x86-64 encodings (immediate, `[reg+disp]`, negative disp8, RIP-relative) into a committed page and a `Region` over it, asserting the decoded value, narrowing, RIP-relative absolute resolution, the always-decode (never the stale `nominal`) contract, and the typed failures (`DecodeFailed` / `UnexpectedShape` / `OperandOutOfRange`).
- **Parallel batch scanner tests:** `tests/test_scanner_parallel.cpp` is a deliberate second suite for the scan module's parallel batch path (the private `detail::scan_regions_batch` / `scan_module_batch` engine primitive in `src/internal/scan_batch.cpp`), kept separate from `test_scanner.cpp` to isolate the threading concerns, mirroring the `test_memory_chain.cpp` split. It plants unique LCG-derived signatures into committed pages and covers `scan_regions_batch` / `scan_module_batch`: empty-batch, per-item input-order preservation regardless of address order, the fail-closed cases (null / empty pattern, zero occurrence, no match), per-item Nth-occurrence, `detail::ScannerKind` selection (a data-only page is invisible to `Executable` but reached by `Readable`), module-range confinement and the invalid-range fail-closed path, and the worker-count knob (`1` serial, a fixed pool, an over-count clamp, and auto) all returning identical results across a many-pattern sweep that exercises the concurrent path. The same suite covers `scan::resolve_batch` equivalence against the serial whole-process, module-scoped, and prologue-fallback resolve paths, including typed per-request failures, plus a `StringXref`-tier request resolving identically to the serial path (the batch resolver dispatches to the same serial resolve machinery). It also pins the `noexcept`-batch never-terminate contract under injected out-of-memory (the thread-local `dmk_test::AllocFailScope` injector in `tests/test_alloc_probe.*`, driving a serial batch so the failures land on the test thread): a container-allocation failure reports the whole-batch failure on the OUTER `Result` (`Error{OutOfMemory}`) without a throw escaping, per-request `bad_alloc` leaves the outer `Result` a value and degrades each inner slot to `Error{OutOfMemory}`, and a white-box `ForkJoinTest` case proves the generic driver keeps the fail-closed seed when a per-item resolve throws anything else (`hook::install_all` has the twin `HookInstallAll.AllocFailureReturnsOutOfMemoryWithoutEscaping` in `test_hook.cpp`; both entry points also carry a `static_assert(noexcept(...))` pin). The resolve tier modes themselves -- `Mode::RttiVtable` and `Mode::StringXref` resolving in a module range, falling through to a byte candidate on backend ambiguity, and returning `NoMatch` on an all-miss -- are pinned in `tests/test_scan_resolve.cpp`.
- **v4 scan resolver tests:** `tests/test_scan_resolve.cpp` covers the `scan.hpp` resolver surface (`scan::resolve` / `resolve_batch` / `scan` / `unchecked::find_pattern`, the `Candidate` factories and factory-only invariants, `ScanRequest` / `OwnedScanRequest` / `borrow`, `order_candidates`, and the `or_null` / `address_or` flattens). It plants distinctive signatures into committed heap pages (and a `VirtualAlloc`'d execute-readable page for the prologue path) and pins Direct and RipRelative resolution, per-request uniqueness (fall-through on ambiguity, `require_unique = false` accepting the first match), scope confinement, the `EmptyCandidates` / `InvalidRange` / `NoMatch` fail-closed codes, RttiVtable / StringXref tier fall-through, `CandidateOrder::UniqueFirst` partitioning, batch per-request ordering, and hooked-prologue E9 recovery plus the short-tail `PrologueFallbackNotApplicable` path.
- **String-reference xref tests:** `tests/test_string_xref.cpp` covers `scan::find_string_xref` across both phase-2 modes. It builds three synthetic images: a single RWX page (`SyntheticImage`) for the default narrow shape scan; a split code/data image (`SplitImage`: an execute-readable code page plus a readable `PAGE_READWRITE` data page) so the broad Zydis sweep only ever decodes the code page; and a four-page `GappedCodeImage` whose executable region is split by a reserved interior page, the only fixture that makes `collect_executable_windows` return more than one window. Cases cover the narrow `lea` / `mov` resolves, the broad-only shapes (`cmp [rip+d], imm`, `push [rip+d]`, a no-REX `lea`, and the REX `lea` superset), the invariant that broad mode keeps default all-offset shape-scan coverage, the decode-failure byte-restart recovery, the `Utf16le` widening and the `require_terminator` prefix guard, the `EnclosingFunction` prologue back-scan (hit and miss), the `StringPointerSlot` store-xref mode (resolves the cached-pointer slot of a `lea reg, [rip+string]; mov [rip+slot], reg` pair, with the r8..r15 REX.R reconstruction, the register-mismatch / out-of-window / mov-load / broad-only `StoreNotFound` rejections), cross-window resolve and ambiguity accumulation, the invariant that the non-executable data page is never swept as code, and every fail-closed `ErrorCode` (`EmptyQuery` / `InvalidRange` / `StringNotFound` / `StringAmbiguous` / `NoReference` / `AmbiguousReference` / `FunctionNotFound` / `StoreNotFound`).
- **Anchor registry:** `tests/test_anchor.cpp` covers the anchor registry (suites AnchorTest, AnchorFingerprintTest, AnchorProfileTest): the Manual / CodeOperand / RipGlobal / StringXref / VtableIdentity backend dispatch, Quorum corroboration (independence checks, agreement, tolerance, nesting), validators and the require_validator policy, assess_quality, the content-based anchor_fingerprint (hashes compiled Pattern content, not AOB source text), apply_profile, the deny-list, and serial-vs-parallel report agreement. The old test_anchors.cpp / test_profile.cpp names are gone; the 2 profile order_candidates + 1 candidate_order_to_string cases were retired as superseded by the scan suite, and the 3 AOB-text fingerprint cases were re-authored against compiled content.
...
- **VMT hook test lifetime:** a `VmtHook` restores the object's vptr in its destructor, so it must be destroyed *before* the target object it was cloned from. Declare the handle AFTER the target (`auto target = ...; VmtHook vh = ...`) so reverse-order local destruction restores the vptr first, or scope the handle so it drops before the target. Any global the detour reaches (e.g. the live `VmtHook*` a method detour calls `original<Fn>(index)` through) must be cleared before the handle drops. Do not leave a hooked test-body object to outlive its handle on a cloned vptr.
- **VMT hook pre-flight symmetry:** The VMT hook path exposes the same operational policy knobs the inline hook does via `VmtOptions`: `fail_if_already_hooked` refuses a `vmt_for`/`apply_to` whose vptr already points at a clone owned by this kit (a silent double-clone on a layered mod is a real bug class), and `fail_on_non_function_pointer` pre-flight-decodes the first byte of the original vtable slot and refuses 0xCC/0xCD/0xC2/0xC3/0x00 plus same-module jump-stub slots. Both default to off. Tests in `tests/test_hook.cpp` (`VmtHook*` / `VmtOptions*`) pin both knobs.
- **Layered inline/mid hook teardown order:** Inline and mid hooks layered on one target address must be unwound newest-first, the same invariant the VMT path enforces. SafetyHook's `disable()` copies a hook's saved prologue bytes back over the target, and a hook created on an already-hooked address saves the live prologue (a jump to the first detour) as *its* original; restoring oldest-first would rewrite the entry to jump into the older hook's about-to-be-freed trampoline (a use-after-free). In v4 this is a documented RAII contract: destroy layered same-target handles newest-first. There is no manager reverse-walk; instead the process-wide `src/internal/hook_ledger.hpp` (a small mutex over target/vptr sets, not a public registry) tracks per-target install order and flags an out-of-order release (`is_target_hooked` plus the release-order check). A same-address create is refused under `Options::fail_if_already_hooked`, which is the ledger exact-match (the prologue-byte heuristic still covers foreign-module hooks). Tests in `tests/test_hook.cpp` pin this.
- **Drift manifest tests:** `tests/test_drift_manifest.cpp` covers `detail/drift_manifest.hpp`: round-trip of a DriftEntry report through serialize/parse (including negative offsets and the owned-name-survives-source-destruction guarantee), the empty-report header-only case, file round-trip, and the fail-closed parse errors (missing header, wrong field count, non-numeric offset). The file reader distinguishes an open failure (`ErrorCode::FileOpenFailed`, a missing path) from a present-but-corrupt manifest (`ErrorCode::MissingHeader`, including an opened-but-empty file); parse returns `Result<std::vector<DriftRecord>>` and errors are compared via `.error().code`. The former `ManifestError` enum folded into `ErrorCode` (`ErrorCategory::Manifest`); `to_string(ErrorCode)` totality and distinctness cover those codes.
- **Diagnostics tests:** `tests/test_diagnostics.cpp` covers `diagnostics.hpp` (the per-subsystem intentional-leak counters): per-subsystem increment isolation, accumulation, the cross-subsystem total, the out-of-range `LeakSubsystem::Count` no-op, and reset. The instrumented loader-lock teardown sites themselves are not reachable from a normal test run; the counter contract is verified directly. A `DiagnosticsEventBusTest` suite in the same file covers the process-wide event bus: `scanner_faults()` / `hook_lifecycle()` each return one stable dispatcher instance, a subscribed handler receives an emitted `ScannerFaultEvent` / `HookLifecycleEvent` with its payload intact, and an RAII unsubscribe stops delivery. The hook-lifecycle integration (a real create / enable / disable / remove cycle emits the four typed transitions in order, a Mid create and VMT create/remove report their hook kinds, and failed or idempotent operations emit nothing) is pinned in `tests/test_diagnostics.cpp` (`DiagnosticsHookLifecycleTest`) and `tests/test_hook.cpp`; the scanner-fault wiring is asserted opportunistically inside the MSVC-only `ScannerRegionGuard.SurvivesConcurrentDecommitMidSweep` race test (every emitted event must carry a positive skipped-region count and the exact scanned window).
- **Diagnostics snapshot tests:** the former `tests/test_diagnostics_dump.cpp` merged into `tests/test_diagnostics.cpp` (suite DiagnosticsSnapshotTest). It covers `diagnostics::collect` -> `Snapshot` (now in `diagnostics.hpp`): empty inputs produce an all-zero snapshot, the leak counters fold in per subsystem and as a total, the drift summary counts healed vs failed entries over a hand-built `DriftEntry` report, the hook-population tally (hooks_total / hooks_active / hooks_disabled) is now restored and delta-tested around a real hook, and the anchor-quality roll-up is covered (assess_quality over a hand-built ResolvedAnchor report).
...
| hook (free functions + RAII Hook/VmtHook) | No central registry. Each `Hook` owns a DMK per-hook `std::recursive_mutex` that serializes `call()` / `enable()` / `disable()` / `~Hook`, so a teardown cannot free the trampoline mid guarded-call; `enable()`/`disable()` drive an atomic CAS status machine. A process-wide `src/internal/hook_ledger.hpp` (a small mutex over target/vptr sets, not a public registry) backs exact duplicate detection (`fail_if_already_hooked`) and layered-same-target teardown-order tracking. The destructor applies the loader-lock leaf discipline: under the loader lock it pins the module and `record_intentional_leak`s instead of restoring | N/A (install/teardown is setup/control-plane; `call()` is serialized by the per-hook recursive mutex) |
...
- `Region::own()` / `Region::host()` -- magic-static cached, single atomic load on the fast path
