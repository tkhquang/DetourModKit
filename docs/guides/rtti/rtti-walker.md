...
| `rtti::find_in_pointer_table(table, n, expected, vtable_cache?, stride?)` | The first object whose vtable matches a mangled name. Optional cache slot (see the pattern below). |
...
| `rtti::vtables_for_type(mangled, out, cap, range?)` | The class can be multiply or virtually inherited and you want every sub-object vtable that shares the name, not only the primary. |
...
`read<dmk::Address>` is deliberate. `Address` is opted into the representation-safe read domain and is pinned to exactly one `std::uintptr_t`. A foreign vtable pointer therefore lands directly in the addressing vocabulary, not in a raw integer that every later call has to re-wrap.
...
It sweeps the module's readable, non-executable sections for the COL whose `TypeDescriptor` name matches, then returns the vtable whose `vtable[-1]` meta-slot points back at that COL. MSVC keeps vtables and their RTTI meta-pointers in those sections (`.rdata` for a normal `/GR` image, `.data` for a packed or section-merged one). Every candidate is re-validated through the same COL prelude the forward walker uses, so a coincidental pointer-shaped match is rejected, not returned.

A key on the name rather than the vtable header is deliberate. The `TypeDescriptor` name string is plain ASCII and fully ASLR-invariant. That is a far stronger anchor than a vtable. A vtable's entries are relocated pointers that shift with the ASLR slide, and its bytes change per launch.
...
- The vtable does not fall inside any loaded module (the loader lookup yields an invalid range), or the image base recovered from the COL (`col_addr - pSelf`) disagrees with the loader-reported module base. Either is the signature of a forged or relocated structure.
- The mangled-name buffer at the resolved address faults on the first page.

None of these raise an exception. The caller can treat all failure modes uniformly through the optional or bool return.

To tell a genuine miss from a module that has no MSVC RTTI to search (a `/GR-` host, a still-packed image, or a data-only module), call `rtti::region_has_rtti(range)`. A `true` means the module carries at least one record, not that your specific type resolves. A `/GR-` executable that links a `/GR` CRT or middleware returns `true` off those library COLs while an executable-owned type still needs the raw-byte fallback. A `false` is the "no resolvable RTTI record was found, fall back to `scan::find_string_xref` / `scan::read_code_constant`" signal, and it is safe to act on. It is not by itself proof of absence. If the sweep did not complete (a faulted section header or an unreadable page), a record can exist in the un-swept region. When that distinction matters, call `rtti::region_rtti_presence(range)`, which returns `RttiPresence::Incomplete` for a truncated sweep instead of a collapse into a bare `false`. It is a setup/control-plane sweep like `vtable_for_type`, so call it once after a miss, never per frame.

## MinGW support

The walker works correctly on both MSVC and MinGW builds of DetourModKit against MSVC-compiled binaries (the typical use case for game mods). The underlying guarded read engine behind `memory::read_into` uses `__try` / `__except` on MSVC and the process-wide vectored fault guard on MinGW. Both toolchains fail closed on unreadable RTTI pages and produce identical results for stable game state. MSVC remains faster because its x64 SEH tables add no success-path setup.
