...
**AVX-512 tier.** The intrinsics are confined to one function by a per-function `target` attribute, with no global `/arch:AVX512` or `-mavx512`. The rest of the library keeps its AVX2 baseline, and the produced binary still runs on a CPU without AVX-512. The tier is selected only when a runtime `CPUID` and `XGETBV` check confirms CPU and OS support for AVX-512F and AVX-512BW. Otherwise the scanner falls back to AVX2. `scan::active_simd_level()` reports the tier in use. The `>= 30%` throughput gate is hardware-specific and can only be measured on a real AVX-512 host. Per-tier correctness runs under Intel SDE on every push to main via [simd-tier-correctness.yml](.github/workflows/simd-tier-correctness.yml).
