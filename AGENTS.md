...
- `[B-86]` `[SAFETY]` **Code on arbitrary host threads must not access `thread_local`.** MinGW lowers it to emutls, which calls `abort()` after first-touch OOM. Install code must reserve a Win32 TLS index, and the last owner of that index must return it. `scripts/check_emit_tls.py` proves compliance. [docs/design/input.md](docs/design/input.md) `[B-86]` owns the rationale.
