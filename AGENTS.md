...
include/DetourModKit/    # Public headers -- one per module
  ...
  hook_manager.hpp       # Hook wrapper (inline, mid, VMT-object). SafetyHook is confined to src/internal/hook_backend.hpp; a mid-hook detour takes an opaque hook::MidContext read/written through gpr() / stack_pointer() / instruction_pointer() / xmm() accessors instead of naming the backend Context. Per-method VMT hooking (hook_vmt_method / with_vmt_method) is deferred to a later release.
  ...
  memory.hpp             # v4 memory surface (namespace DetourModKit::memory): read<T>/read_into/write<T>/write_bytes (Result<T>) over Address/Region, sharded region cache, walk (fault-guarded pointer chain), is_plausible_ptr, ProtectGuard, module_of/is_module_loaded
  ...
  diagnostics_dump.hpp   # One-call aggregator (Diagnostics::collect -> Diagnostics::Snapshot) bundling the leak counters, get_hook_counts(), and heal_report() summaries into one struct
...
Do not add `memory::is_readable()` or `memory::is_writable()` before every field read in hook callbacks. Use those predicates for setup validation and diagnostics. Use `memory::walk` for unstable live game pointers, and use `memory::unchecked::read` only when the caller can prove the pointer chain is live for the current frame. For per-frame WRITES through a resolved address, use the guarded `memory::write<T>` / `write_bytes` family (which auto-unprotect: a write to a read-only page succeeds, and protection is changed only when the fast guarded write faults); for a one-shot code patch, make the protection change explicit with a `memory::ProtectGuard`. The full pattern -- worked examples, the primitive selection table, and the anti-patterns to remove -- lives in [docs/misc/hot-path-memory.md](docs/misc/hot-path-memory.md).
...
- `memory::write<T>()` / `write_bytes()` -- guarded per-frame WRITES to game memory (a camera transform, a player field), the write counterpart of the reads. MSVC uses one `__try` frame, and MinGW x64 uses the vectored-handler copy path with a fallback through VirtualQuery plus WriteProcessMemory, returning `Result<void>` so a stale address fails closed instead of faulting the host. Both auto-unprotect: a write to a read-only page succeeds, and protection is changed (and restored) only when the fast guarded write faults. For a one-shot code patch that must flip page protection up front, take a `memory::ProtectGuard` explicitly
