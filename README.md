...
- `write<T>(Address, value)` / `write_bytes(Address, std::span<const std::byte>)` - the WRITE counterpart of the read family: a typed value or raw bytes written to game memory, returning `Result<void>`. They auto-unprotect (a write to a read-only page succeeds; protection is changed only when the fast guarded write faults). Use these for per-frame data writes (a camera transform, a player field); a one-shot code patch is the same `write_bytes`, made explicit with a `ProtectGuard`
