...
- `test_mid_route_retention.cpp` is the route-retention host (`Lifecycle.PublishedMidRouteRetainsOneBlockPerHook`). A whole-address-space `VirtualQuery` walk proves that five cycles of six published mid hooks each retain six private executable regions of one allocation granularity. It also proves that `route_retention_stats()` charges three granules per hook, and that six inline hooks or six never-enabled mid hooks retain nothing. The host names the backend for the counters. A retention change in either direction fails it.
