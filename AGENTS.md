...
include/DetourModKit/    # Public headers -- one per module
  ...
  rtti.hpp               # MSVC RTTI walker (namespace DetourModKit::rtti, over Address / Result<T>): type_name_of, vtable_is_type, find_in_pointer_table (std::optional<Address>, caller-owned std::atomic<Address> cache) + reverse name-to-vtable (vtable_for_type / vtables_for_type -> std::optional<Address>, self-contained TypeIdentity that owns its mangled name)
