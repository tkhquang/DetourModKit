...
// This translation unit owns the test binary's single replacement of the global allocation operators. The throwing
// forms tally a per-thread counter that allocation-sensitive tests read through a before/after delta; the aligned
// (std::align_val_t) forms are deliberately left at their defaults, so over-aligned allocations stay on the runtime's
// own consistent new/delete pair and never cross-free against these.
namespace
{
