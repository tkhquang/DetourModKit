...
- `bootstrap_probe_dll.cpp` / `test_bootstrap_module_ref.cpp` -- the raw T-BOOTSTRAP lifecycle proof.
- `input_loader_detach_dll.cpp` / `test_input_loader_detach.cpp` -- the raw T-INPUT-LOADER proof.
...
- `xinput_detour_rundown.cpp` -- the fresh-process host for XInput hook lifetime and pair integrity.
  Each CLI mode maps to one `Lifecycle.XInput*` CTest case.
  The `process-exit-storage` mode proves the B-100 XInput exit boundary.
  The suite covers install failure, byte reconciliation, pair recovery, teardown, and module reference balance.
  It also proves route unwind metadata and capacity limits.
  Host-dependent modes return skip code 77 when no subject exists.
  This test target names the backend because its route proofs inspect generated regions.
