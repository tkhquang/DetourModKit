 ...
 *
 * The code-side twin of the RTTI self-heal: an AOB cascade lands on an instruction, the instruction is decoded, and the
 * requested operand's immediate or memory displacement is returned as the CURRENT value. The caller's nominal is never
 * a short-circuit, so a same-shape / different-value drift (for example an array stride 232 -> 240) is reported as the
 * new value, which is the entire point of re-deriving it.
 *
 * Zydis is confined to this translation unit: no public DetourModKit header exposes a Zydis type. An installed-package
 * consumer therefore only links
 * DetourModKit (which already links Zydis statically) and never needs Zydis headers on its own include path.
 */
...
namespace DetourModKit
{
    namespace
    {
        // Narrows an already-64-bit-sign-extended value to @p byte_width bytes and re-sign-extends from that width, so
        // a deliberately narrowed negative value (for example a disp8 of -1) stays negative instead of becoming a large
        // positive number. @p byte_width 0 returns the value verbatim, since
    ...
    std::expected<std::int64_t, Scanner::ResolveError> Scanner::read_code_constant(const CodeConstant &cc,
                                                                                   Memory::ModuleRange range)
    {
        // Resolve the instruction site through the existing module-scoped cascade and propagate its typed failure
        // verbatim (EmptyCandidates, NoMatch, InvalidRange, ...).
        ...
        // Read a full maximum-length instruction window, clamped to the module so the read never runs past the end of
        // the image, behind a fault guard. A truncated window that fails to decode is reported as DecodeFailed below.
        ...
        // implicit/hidden operands (flags, implicit registers, stack writes), which would make a fixed operand_index
        // drift between mnemonics.
        ...
        // MemoryDisplacement. A register-indirect operand with no displacement (for example plain `[rcx]`) carries no
        // constant to read.
        ...
        if (operand.mem.base == ZYDIS_REGISTER_RIP)
        {
            ...
            // instruction, not the absolute constant the caller wants. Resolve it to the absolute target so the return
            // value is meaningful rather than a misleading relative offset.
            ZyanU64 absolute = 0;
