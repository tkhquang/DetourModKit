...
/**
 * @file x86_decode.hpp
 * @brief Minimal jump-shape decoders for the nested-hook detection and prologue-recovery paths.
 * @details Each decoder copies the candidate instruction bytes into a local buffer under a single fault guard
 *          (guarded_read_bytes), then inspects the copy. Read-then-test on a local buffer closes the time-of-check to
 *          time-of-use gap that is_readable + a raw dereference leaves open: on these paths the target page is most
 *          likely to change protection or unmap between the probe and the access. A faulting read returns nullopt
 *          rather than faulting the host. These recognise a single jump shape each; they are not a general decoder.
 */
